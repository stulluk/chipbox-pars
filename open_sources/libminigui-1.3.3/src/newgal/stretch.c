/*
**  $Id: stretch.c,v 1.2 2003/09/04 06:02:53 weiym Exp $
**  
...
static int generate_rowbytes(int src_w, int dst_w, int bpp)
{
	...
	switch (bpp) {
		...
		break;
	    default:
		GAL_SetError("ASM stretch of %d bytes isn't supported\n", bpp);
		return(-1);
	}
	pos = 0x10000;
	inc = (src_w << 16) / dst_w;
	...
	*eip++ = RETURN;
	...
	if ( eip > (copy_row+sizeof(copy_row)) ) {
		GAL_SetError("Copy buffer overflow");
		return(-1);
	}
	return(0);
}
                    ...
                    GAL_Surface *dst, GAL_Rect *dstrect)
{
	...
	if ( src->format->BitsPerPixel != dst->format->BitsPerPixel ) {
		GAL_SetError("Only works with same format surfaces");
		return(-1);
	}
	...
	if ( srcrect ) {
		if ( (srcrect->x < 0) || (srcrect->y < 0) ||
			...
			GAL_SetError("Invalid source blit rectangle");
			return(-1);
		}
	} else {
		full_src.x = 0;
	...
	if ( dstrect ) {
		if ( (dstrect->x < 0) || (dstrect->y < 0) ||
			...
			GAL_SetError("Invalid destination blit rectangle");
			return(-1);
		}
	} else {
		full_dst.x = 0;
