/*
** $Id: menu.c,v 1.47 2003/09/04 03:15:07 weiym Exp $
**
...
BOOL GUIAPI HiliteMenuBarItem (HWND hwnd, int pos, UINT flags)
{
    ...
    if (flags == HMF_UPITEM) {
       Draw3DUpThinFrame (hdc, rc.left - inter, rc.top - 3, 
                           rc.right + inter, rc.bottom - 1, PIXEL_invalid);
    }
    else if (flags == HMF_DOWNITEM) {
       mnuDrawMenuBarItem (hdc, pmi, rc.left + 1, rc.top + 1, 
            RECTW (rc) - 2, RECTH (rc) - 2);
       Draw3DDownThinFrame (hdc, rc.left - inter, rc.top - 2, 
                           rc.right + inter, rc.bottom - 2, PIXEL_invalid);
    }
...
#endif
#else
    Draw3DUpFrame (HDC_SCREEN, ptmi->rc.left, ptmi->rc.top, 
