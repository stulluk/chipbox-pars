/*
** $Id: combobox.c,v 1.50 2003/11/23 04:09:08 weiym Exp $
**
...
static BITMAP bmp_downarrow;
static BITMAP bmp_updownarrow;
...
BOOL RegisterComboBoxControl (void)
{
    ...
    if (!LoadSystemBitmap (&bmp_downarrow, SYSBMP_DOWNARROW))
        return FALSE;

    if (!LoadSystemBitmap (&bmp_updownarrow, SYSBMP_UPDOWNARROW))
        return FALSE;
...
inline static void GetButtonRect (HWND hwnd, int *x, int *y, int *w, int *h)
{
    PCOMBOBOXDATA pData;
    PCONTROL pCtrl;
    ...
    *x = pData->ButtonRect.left;
    *y = pData->ButtonRect.top;
    *w = pData->ButtonRect.right-*x;
    *h = pData->ButtonRect.bottom-*y;
}

static BOOL ComboBoxDownButton (HWND hwnd, HDC hdc)
{
    int x, y, w, h;
    PCOMBOBOXDATA pData;
    PCONTROL pCtrl;
    ...
    GetButtonRect (hwnd, &x, &y, &w, &h);

    if (pData->ButtonBmp)
        FillBoxWithBitmap (hdc, x, y + WIDTH_EDIT_BORDER, 0, 0, pData->ButtonBmp);
...
static int ComboBoxCtrlProc (HWND hwnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
        ...
        {
            RECT rect;
            int  height, width, edit_width, list_height, btn_width;
            int  tmpx, tmpy;
            ...
            //height = GetMainWinMetrics (MWM_ICONY) 
            height = GetSysCharHeight ()
                    + MARGIN_EDIT_TOP + MARGIN_EDIT_BOTTOM + (WIDTH_EDIT_BORDER<<1);
            width = rect.right - rect.left;

            btn_width = GetMainWinMetrics (MWM_ICONX);
            edit_width = width - btn_width - INTER_EDIT_BUTTON;
            ...
            case CBS_SPINLIST:
                pData->ButtonBmp = ARROWBMP_UPDOWN;
                list_height = 0;
                pData->spin_pace = 1;
            ...
            default:
                list_height = (pCtrl->dwAddData > 0) ? pCtrl->dwAddData : DEF_LISTHEIGHT;
                pData->ButtonBmp = ARROWBMP_DOWN;
                pData->ListBoxRect.left = rect.left;
                pData->ListBoxRect.right = rect.right;
                pData->ListBoxRect.top = rect.top + height;
                pData->ListBoxRect.bottom = rect.top + height + list_height;
                break;
            }
            ...
            pData->ButtonRect.left = rect.right - btn_width;
            pData->ButtonRect.right = rect.right;
            pData->ButtonRect.top = rect.top;
            pData->ButtonRect.bottom = rect.top + height;
            break;
        }
        ...
        case MSG_SIZECHANGING:
        {
            ...
            int height = GetMainWinMetrics (MWM_ICONY) 
                    + MARGIN_EDIT_TOP + MARGIN_EDIT_BOTTOM + (WIDTH_EDIT_BORDER<<1);
            ...
            if ((dwStyle & CBS_TYPEMASK) == CBS_SIMPLE)
                rcResult->bottom += 
                        (pData->ListBoxRect.bottom - pData->ListBoxRect.top + INTER_EDIT_LISTBOX);
            return 0;
        }
        ...
        { 
            ...
            if (((dwStyle & CBS_TYPEMASK) > CBS_AUTOSPIN) && PtInRect (&pData->ButtonRect, x, y))
        ...
        { 
            ...
            if (PtInRect (&pData->ButtonRect, x, y)) { 
                BOOL down_button = FALSE;
                if (y > ((pData->ButtonRect.bottom + pData->ButtonRect.top) >> 1)) {
                    down_button = TRUE;
                }
                ...
                case CBS_AUTOSPIN:
                    SetCapture (hwnd);
                    SendMessage (hwnd, CB_SPIN, down_button, 0);
                    SetAutoRepeatMessage (hwnd, CB_FASTSPIN, down_button, 0);
                    break;
