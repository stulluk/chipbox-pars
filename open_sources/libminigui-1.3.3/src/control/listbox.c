...
BOOL RegisterListboxControl (void)
{
    ...
    if (!LoadSystemBitmap (&bmp_checkmark, SYSBMP_CHECKMARK))
        return FALSE;
...
static BOOL lstInitListBoxData (PCONTROL pCtrl, LISTBOXDATA* pData, int len)
{
    int i;
    PLISTBOXITEM plbi;
...
static void lstInvalidateItem (HWND hwnd, PLISTBOXDATA pData, 
                int pos, BOOL fEBk)
{
    RECT rcInv;
...
static int ListboxCtrlProc (HWND hwnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        case MSG_CREATE:
            ...
            if (!lstInitListBoxData (pCtrl, pData, DEF_LB_BUFFER_LEN)) {
                free (pData);
                return -1;
            }
        ...
        case MSG_PAINT:
        {
            ...
            pData = (PLISTBOXDATA)pCtrl->dwAddData2;
            lstOnDrawSListBoxItems (hdc, dwStyle, pData, RECTW (rc));
        ...
        case MSG_CHAR:
        {
            ...
            if (index >= 0) {
                ...
                if (!(dwStyle & LBS_MULTIPLESEL)) {
                    int oldSel = lstSelectItem (dwStyle, pData, index);
