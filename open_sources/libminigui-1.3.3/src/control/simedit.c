/*
** $Id: simedit.c,v 1.20 2003/11/23 04:09:08 weiym Exp $
**
            ...
            PSIMEDITDATA simed)
{
    ...
    while (TRUE) {
        if ((endPos - newStartPos) * GetSysCharWidth () < nOutWidth)
            break;
...
static int edtGetOffset (const SIMEDITDATA* simed, int x)
{
    ...
    x -= simed->leftMargin;
    for (i = simed->startPos; i < simed->dataEnd; i++) {
        if ((nTextWidth + (GetSysCharWidth() >> 1)) >= x)
            break;
...
static void edtOnEraseBackground (HWND hWnd, DWORD dwStyle, HDC hdc, const RECT* pClipRect)
{
    RECT rcTemp;
    BOOL fGetDC = FALSE;
    PSIMEDITDATA simed = (PSIMEDITDATA) GetWindowAdditionalData2(hWnd);
    ...
    if (dwStyle & ES_BASELINE) {
        ...
        DrawHDotLine (hdc, 
                    simed->leftMargin, 
                    simed->topMargin + GetSysCharHeight () + 1,
                    RECTW (rcTemp) - simed->leftMargin - simed->rightMargin);
    }
...
{   
    ...
    switch (message)
    {
        case MSG_CREATE:
            ...
            pCtrl->pLogFont = GetSystemFont (SYSLOGFONT_WCHAR_DEF);
            break;
        ...
        case MSG_SETFOCUS:
            ...
            SetCaretPos (hWnd, simed->caretOff * GetSysCharWidth () + simed->leftMargin, 
                simed->topMargin);
        ...
        case MSG_KEYDOWN:
        {
            ...
            switch (LOWORD (wParam))
            {
                ...
                case SCANCODE_CURSORBLOCKRIGHT:
                {
                    ...
                    if (simed->caretOff * GetSysCharWidth () 
        ...
        case EM_SETCARETPOS:
        {
            ...
            if (char_pos >= 0 && char_pos <= simed->dataEnd) {
                ...
                if (simed->status & EST_FOCUSED)
	                SetCaretPos (hWnd, simed->caretOff * GetSysCharWidth () 
        ...
        case EM_SETPASSWORDCHAR:
            ...
            if (simed->passwdChar != (int)wParam) {
                if (dwStyle & ES_PASSWORD) {
                    simed->passwdChar = (int)wParam;
                    InvalidateRect (hWnd, NULL, TRUE);
                }
            }
            return 0;
