/*
** $Id: propsheet.c,v 1.34 2003/09/04 02:40:36 weiym Exp $
**
...
static int PropSheetCtrlProc (HWND hwnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
        case MSG_CREATE:
        {
            if (!(pData = calloc (1, sizeof (PROPSHEETDATA))))
                return -1;
        ...
        {
            int x, y;
            RECT title_rc = {0, 0, 0, pData->head_rc.bottom};
            PPROPPAGE page;
        ...
        case MSG_PAINT:
        {
            ...
            DrawFlatControlFrameEx (hdc, 0, pData->head_rc.bottom - 2,
                                      pData->head_rc.right, 
                                      pCtrl->bottom - pCtrl->top,
                                      PIXEL_invalid, 1, TRUE);
#else
            Draw3DUpThickFrame (hdc, 0, pData->head_rc.bottom - 2, 
                                      pData->head_rc.right, 
                                      pCtrl->bottom - pCtrl->top,
                                      PIXEL_invalid);
#endif
            ...
            FillBox (hdc, 0, 0, pData->head_width, pData->head_rc.bottom - 3);
