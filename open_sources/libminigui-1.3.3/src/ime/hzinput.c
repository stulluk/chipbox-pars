...
static HWND sg_hTargetWnd = HWND_DESKTOP; // target window of ime.
...
static BOOL hz_filter(unsigned char key, LPARAM lParam)
{
    ...
    if (IsHanziInput) {
        ...
        if (IsFullPunc) {
            ...
            if (len != 0) {
                writemsg(cpunc, len, lParam, TRUE);
                ClrIn();
                return TRUE;
            }
        }
 ...
 *                        extern function                                *
 *************************************************************************/
void writemsg (BYTE *buffer, int len, LPARAM lParam, BOOL bDByte)
{
    WORD wDByte;
    int i;
...
void outchar (unsigned char c, LPARAM lParam)
{
    int scancode = 0;
