/*
** $Id: tf-stb.c,v 1.15 2003/11/21 12:15:37 weiym Exp $
**
...
static BYTE get_funccode (BYTE serial_code, int code)
{
	...
	switch (code) {
	case PANEL:
		return panel_funcmap[i][j];
	case REMOTER:
		return remoter_funcmap[i][j];
	case KEYBOARD:
		return kb_funcmap[i][j];
	default:
		return -1;
	}
