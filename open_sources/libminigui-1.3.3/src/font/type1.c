/*
** $Id: type1.c,v 1.21 2003/09/25 04:08:55 snig Exp $
** 
                ...
                const unsigned char* mstr, int n, int cExtra)
{
	...
	for ( i = 0; i <= n-1; i++ ) {
		width = T1_GetCharWidth ( type1_info->font_id, * ( mstr+i ) );
		sum += width;
	}

	sum = sum * type1_inst_info->csUnit2Pixel + cExtra * ( n - 1 );
	return sum;
}
                ...
                const unsigned char* mchar, int len)
{
	...
	get_char_bitmap(logfont,devfont,mchar,len);
...
#if 0
static void* get_str_bitmap (LOGFONT* logfont, DEVFONT* devfont,
            const unsigned char* str, int len,long spaceoff)
{
	GLYPH * glyph;
	int modflag;
	int style;
	...
	if(type1_inst_info->last_bitmap_str)
		if(!strncmp(type1_inst_info->last_bitmap_str,str,len))
				return type1_inst_info->last_bits;
	modflag = 0;
	style	= logfont->style;

	modflag |= T1_KERNING ;
	if ( style & FS_UNDERLINE_LINE ) {
		modflag |= T1_UNDERLINE;
	}

	if ( style & FS_STRUCKOUT_LINE ) {
		modflag |= T1_OVERSTRIKE;
	}
	
	glyph = T1_SetString ( type1_info->font_id, (char*)str, len, spaceoff,modflag,\
			type1_inst_info->size , type1_inst_info->pmatrix) ;
	...
	type1_inst_info->last_bitmap_str = (char * ) calloc ( len, sizeof(char) );
	strncpy(type1_inst_info->last_bitmap_str, str, len);
    	type1_inst_info->last_ascent = glyph->metrics.ascent;
    	type1_inst_info->last_descent = glyph->metrics.descent;
    	type1_inst_info->last_leftSideBearing = glyph->metrics.leftSideBearing;
    	type1_inst_info->last_rightSideBearing = glyph->metrics.rightSideBearing;
	...
	/*
	 * if not set it to null, T1_SetString will free it next time it is called,
	 * we do so to keep it by ourself
	 * */ 
	glyph->bits = NULL;
...
#if 0
static void* get_str_pixmap (LOGFONT* logfont, DEVFONT* devfont,
            const unsigned char* str, int len, long spaceoff, int * pitch)

/* FIXME: 
 * If u will use the function, please give values to pitch as 
 * I did in get_char_pixmap
 * */
{
	GLYPH * glyph;
	int modflag;
	...
	if(type1_inst_info->last_pixmap_str)
		if(!strncmp(type1_inst_info->last_pixmap_str,str,len))
				return type1_inst_info->last_bits;
	modflag = 0; 
	style	= logfont->style;
	...
	glyph = T1_AASetString ( type1_info->font_id, ( char* )str, len, spaceoff, modflag, 
			type1_inst_info->size , type1_inst_info->pmatrix) ;
	type1_inst_info->last_bitmap_char = -1;
	type1_inst_info->last_pixmap_char = -1;
	if(type1_inst_info->last_bitmap_str){
	...
	type1_inst_info->last_pixmap_str = (char * ) calloc ( len, sizeof(char) );
	strncpy(type1_inst_info->last_pixmap_str, str, len);
	...
	if(!type1_inst_info->last_bits)
    	...
    	type1_inst_info->last_bits = glyph->bits;
	/*
	 * if not set it to null, T1_AASetString will free it next time it is called,
	 * we do so to keep it by ourself
	 * */ 
	glyph->bits = NULL;
...
static DEVFONT* new_instance (LOGFONT* logfont, DEVFONT* devfont, BOOL need_sbc_font)
{
	...
	type1_inst_info->last_pixmap_str  = NULL;	
	c = 'A';
	get_char_bitmap(logfont,new_devfont,&c,1);
