                        ...
                        const BYTE* pXorBits, int colornum)
{
    ...
    if( !(picon->AndBits = malloc(w * h * bpp)) ) {
        free(picon);
        return 0;
    }
    if( !(picon->XorBits = malloc(w * h * bpp)) ) {
        free(picon->AndBits);
        free(picon);
        return 0;
    }
    ...
    if(colornum == 1) {
        ExpandMonoBitmap (hdc, w, h, pAndBits, MYBMP_FLOW_UP, align_32_bits (w >> 3), 
                         picon->AndBits, 0, 0xFFFFFFFF);
        ExpandMonoBitmap (hdc, w, h, pXorBits, MYBMP_FLOW_UP, align_32_bits (w >> 3),
                         picon->XorBits, 0, 0xFFFFFFFF);
    }
    else if(colornum == 4) {
        ExpandMonoBitmap (hdc, w, h, pAndBits, MYBMP_FLOW_UP, align_32_bits (w >> 3), 
                         picon->AndBits, 0, 0xFFFFFFFF);
        Expand16CBitmap (hdc, w, h, pXorBits, MYBMP_FLOW_UP, align_32_bits (w >> 1),
                        picon->XorBits, NULL);
    }
