/* $Id: winbmp.c,v 1.8 2003/11/23 04:09:08 weiym Exp $
**
...
int load_bmp (MG_RWops* fp, MYBITMAP* bmp, RGB* pal)
{
    ...
    MGUI_RWseek (fp, fileheader.bfOffBits, SEEK_SET);
    ...
    bmp->flags = MYBMP_TYPE_BGR | MYBMP_FLOW_UP | MYBMP_RGBSIZE_3;
...
int save_bmp (MG_RWops* fp, MYBITMAP* bmp, RGB* pal)
{
    ...
    switch (bmp->depth) {
        case 4:
            ...
            for (i = bmp->h  - 1; i >= 0; i--) {
                bmpGet16CScanline (bmp->bits + i * bmp->w * 1, scanline, bmp->w);
        ...
        case 8:
            ...
            for (i = bmp->h - 1; i >= 0; i--) {
                bmpGet256CScanline (bmp->bits + i * bmp->w, scanline, bmp->w);
                MGUI_RWwrite (fp, scanline, sizeof (char), scanlinebytes);
            }
        break;
        ...
        default:
            ...
            for (i = bmp->h - 1; i >= 0; i--) {
                bmpGetHighCScanline (bmp->bits + i * bmp->w * bpp, scanline, 
                                bmp->w, bpp, bmp->depth);
                MGUI_RWwrite (fp, scanline, sizeof (char), scanlinebytes);
            }
        break;
