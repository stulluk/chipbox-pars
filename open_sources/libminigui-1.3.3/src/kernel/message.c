...
BOOL RcuKeyMessageDisable = FALSE;  // by kb: 20100408
BOOL RcuKeyControlOn      = FALSE;

/* by kb : 20101229 For Key buffer */
unsigned char MaxKeyInputBufferSize = 1;
...
#define _MSG_STRING   1  /* By KB Kim : 2010_06_17 */
...
/* by kb : 20100408 */
void SetKeyInputControl (BOOL controlOn)
{
	RcuKeyControlOn = controlOn;
}

BOOL GetKeyInputControl(void)
{
	return RcuKeyControlOn;
}

/* by kb : 20101229 For Key buffer */
void SetKeyInputBufferSize (unsigned char bufferSize)
{
	MaxKeyInputBufferSize = bufferSize;
}

unsigned char GetKeyInputBufferSize(void)
{
	return MaxKeyInputBufferSize;
}
/***********************/

int GUIAPI DispatchMessage(PMSG pMsg)
{
	int GUIAPI resultWinProc;
    WNDPROC WndProc;
    PSYNCMSG pSyncMsg;
    ...
    if ( pMsg->pAdd ) // this is a sync message.
    {
        ...
        pSyncMsg->retval = (*WndProc)
                   (pMsg->hwnd, pMsg->message, pMsg->wParam, pMsg->lParam);
        sem_post(&pSyncMsg->sem_handle);
		RcuKeyMessageDisable = FALSE;  /* by kb : 20100408 */
        return pSyncMsg->retval;
    }
	resultWinProc = (*WndProc)(pMsg->hwnd, pMsg->message, pMsg->wParam, pMsg->lParam);
	RcuKeyMessageDisable = FALSE;  /* by kb : 20100408 */
    return resultWinProc;
}
