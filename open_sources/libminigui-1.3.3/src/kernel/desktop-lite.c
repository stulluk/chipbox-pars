/*
** $Id: desktop-lite.c,v 1.48 2003/11/21 02:45:53 weiym Exp $
**
                ...
                int lx, int ty, int rx, int by)
{
    ...
    if (mgIsServer) { // will ignore layer_name and app_name
        RECT rcSub [4];
        int i, n, maxrc = -1, area, maxarea = 0;

        g_rcExcluded = g_rcDesktop;
        g_rcDesktop = g_rcScr;

        n = SubtractRect (rcSub, &g_rcScr, &g_rcExcluded);
        // find max rect
        for (i = 0; i < n; i ++) {
            area = (RECTW (rcSub[i]) * RECTH (rcSub[i]));
            if (area > maxarea) {
                maxarea = area;
                maxrc = i;
            }
        ...
        *SHAREDRES_LAYER_RC = *SHAREDRES_CLI_SCR_RC = rcSub[maxrc];
        ...
        if (IsRectEmpty (&joined_info.desktop_rc)) {
            fprintf (stderr, "JoinLayer: Empty desktop rect.\n");
            goto ret;
        }
        ...
        SelectClipRect (HDC_SCREEN, &g_rcExcluded);
