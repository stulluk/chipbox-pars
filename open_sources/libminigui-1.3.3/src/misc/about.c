/*
** $Id: about.c,v 1.25 2003/09/29 02:52:46 snig Exp $
**
...
static int AboutWinProc (HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
        case MSG_CREATE:
            ...
            if (!(bmpLogo = malloc (sizeof(BITMAP))))
                return -1;

            if (!LoadSystemBitmap (bmpLogo, SYSBMP_LOGO))
                return -1;
        ...
        case MSG_PAINT:
        {
            ...
            FillBoxWithBitmap (hdc, 
                (RECTW(rcClient) - bmpLogo->bmWidth)>>1, 10, 
                0, 0, bmpLogo);
