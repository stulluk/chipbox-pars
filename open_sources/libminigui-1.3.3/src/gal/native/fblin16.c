/*
** $Id: fblin16.c,v 1.22 2003/11/22 11:49:29 weiym Exp $
**
...
static void linear16_putboxmask ( PSD psd, int x, int y, int w, int h, void *buf, gal_pixel cxx)
{
    ...
    for (i = 0; i < h; i++) {
        src1 = (gal_uint8*)src ; 
        endoflinesrc = (gal_uint8*)(src +  w);

        while (src1 < endoflinesrc - 7) {
            gal_uint32 c2 = *(gal_uint32 *) src1;
            if ((c2 & 0xffff) != cxx)
            *(gal_uint16 *) dst = (gal_uint16) c2;
            c2 >>= 16;
            if (c2 != cxx)
            *(gal_uint16 *) (dst + 2) = (gal_uint16) c2;
            
            c2 = *(gal_uint32 *) (src1 + 4);
            if ((c2 & 0xffff) != cxx)
            *(gal_uint16 *) (dst + 4) = (gal_uint16) c2;
            c2 >>= 16;
            if (c2 != cxx)
            *(gal_uint16 *) (dst + 6) = (gal_uint16) c2;
            src1 += 8;
            dst += 8;
        }
        while (src1 < endoflinesrc) {
            gal_uint16 c = *(gal_uint16 *) src1;
            if (c != cxx)
                *(gal_uint16 *) dst = c;
            src1 += 2;
            dst += 2;
