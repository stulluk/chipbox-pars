/*
** $Id: dc.h,v 1.39 2003/08/15 07:41:26 weiym Exp $
**
...
{
    BYTE* dst = pdc->surface->pixels + pdc->surface->pitch * y;
...
#define CHECK_CLI_SCREEN(pdc, rcOutput)                 \
        (!mgIsServer && (pdc->surface == __gal_screen)  \
            && !IntersectRect (&rcOutput, &rcOutput, (const RECT*)&SHAREDRES_CLI_SCR_LX))
...
#define ENTER_DRAWING_NOCHECK(pdc) enter_drawing_nocheck(pdc)

#define LEAVE_DRAWING_NOCHECK(pdc) leave_drawing(pdc)
