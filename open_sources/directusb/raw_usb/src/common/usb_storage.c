...
static void display_int_status(unsigned long tmp)
{
	printf("Status: %s %s %s %s %s %s %s\n",
	       ...
	       (tmp & USB_ST_CRC_ERR) ? "CRC Error" : "", (tmp & USB_ST_BIT_ERR) ? "Bitstuff Error" : "");
}
#endif
/***********************************************************************
...
int usb_stor_BBB_comdat(ccb * srb, struct us_data *us)
{
	...
	printf("dir %d lun %d cmdlen %d cmd %p datalen %d pdata %p\n", dir_in, srb->lun, srb->cmdlen, srb->cmd,
	       srb->datalen, srb->pdata);
	if (srb->cmdlen) {
...
int usb_stor_CB_comdat(ccb * srb, struct us_data *us)
{
	...
	while (retry--) {
		...
		result = usb_control_msg(us->pusb_dev, usb_sndctrlpipe(us->pusb_dev, 0),
					 US_CBI_ADSC, USB_TYPE_CLASS | USB_RECIP_INTERFACE,
					 0, us->ifnum, srb->cmd, srb->cmdlen, USB_CNTL_TIMEOUT * 5);
		...
		if (srb->datalen) {
			...
			USB_STOR_PRINTF("CBI attempted to transfer data, result is %d status %lX, len %d\n", result,
					us->pusb_dev->status, us->pusb_dev->act_len);
			if (!(us->pusb_dev->status & USB_ST_NAK_REC))
				break;
		}		/* if (srb->datalen) */
		else
	...
	case 0x02:		/* Not Ready */
		if (notready++ > USB_TRANSPORT_NOT_READY_RETRY) {
			printf("cmd 0x%02X returned 0x%02X 0x%02X 0x%02X 0x%02X (NOT READY)\n",
			       srb->cmd[0], srb->sense_buf[0], srb->sense_buf[2], srb->sense_buf[12],
			       srb->sense_buf[13]);
			return USB_STOR_TRANSPORT_FAILED;
		}
		else {
...
int usb_stor_get_info(struct usb_device *dev, struct us_data *ss, block_dev_desc_t * dev_desc)
{
...
#endif
#ifdef LITTLEENDIAN
	cap[0] = ((unsigned long) ((((unsigned long) (cap[0]) & (unsigned long) 0x000000ffUL) << 24) |
	...
	cap[1] = ((unsigned long) ((((unsigned long) (cap[1]) & (unsigned long) 0x000000ffUL) << 24) |
