...
printf("dev = %d, part = %d \n", dev, part);
	...
	if (argc == 4)
		ret = file_fat_ls(argv[3], NULL);
	else
		ret = file_fat_ls(filename, NULL);
...
int do_fat_dump(int argc, char *argv[])
{
	u8 block[1024];
	int ret;
	int bknum;
...
int disk_read2(u32 startblock, u32 getsize, u8 * bufptr)
{
	ulong tot;
	block_dev_desc_t *dev_desc;
	...
	tot = dev_desc->block_read(0, startblock + part_offset, getsize, (ulong *) bufptr);
