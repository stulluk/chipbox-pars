...
int disk_read(u32 startblock, u32 getsize, u8 * bufptr)
{
 ...
 */
int compare_sign(char *str1, char *str2)
{
 ...
 */
u32 get_fatent(fsdata * mydata, u32 entry)
{
	u32 bufnum;
	u32 offset;
	u32 ret = 0x00;
	...
	case 16:
		...
		break;
	case 12:{
			u32 off16 = (offset * 3) / 4;
			u16 val1, val2;

			switch (offset & 0x3) {
			case 0:
				ret = FAT2CPU16(((u16 *) mydata->fatbuf)[off16]);
				ret &= 0xfff;
				break;
			case 1:
				val1 = FAT2CPU16(((u16 *) mydata->fatbuf)[off16]);
				val1 &= 0xf000;
				val2 = FAT2CPU16(((u16 *) mydata->fatbuf)[off16 + 1]);
				val2 &= 0x00ff;
				ret = (val2 << 4) | (val1 >> 12);
				break;
			case 2:
 ...
 */
int get_cluster(fsdata * mydata, u32 clustnum, u8 * buffer, unsigned long size)
{
	int idx = 0;
	u32 startsect;
 ...
 */
long get_contents(fsdata * mydata, dir_entry * dentptr, u8 * buffer, unsigned long maxsize)
{
 ...
 * Return 0 on success, -1 otherwise.
 */
u8 get_vfatname_block[MAX_CLUSTSIZE];
int get_vfatname(fsdata * mydata, int curclust, u8 * cluster, dir_entry * retdent, char *l_name)
{
	dir_entry *realdent;
	dir_slot *slotptr = (dir_slot *) retdent;
	u8 *nextclust = cluster + mydata->clust_size * SECTOR_SIZE;
	u8 counter = (slotptr->id & ~LAST_LONG_ENTRY_MASK) & 0xff;
	int idx = 0;

	while ((u8 *) slotptr < nextclust) {
		if (counter == 0)
			break;
	...
	if ((u8 *) slotptr >= nextclust) {
		...
		while ((u8 *) slotptr2 >= get_vfatname_block) {
...
static u8 mkcksum(const char *str)
{
	int i;
	u8 ret = 0;
...
u8 do_fat_read_block[MAX_CLUSTSIZE];	/* Block buffer */
long do_fat_read(const char *filename, 
		void *buffer, 
		unsigned long maxsize, 
		int dols, 
		void (*proc)(const char *fsname, int sz, char attr, void *context))
{
	...
	FAT_DPRINT("Rootdir begins at sector: %d, offset: %x, size: %d\n"
		   "Data begins at: %d\n",
		   mydata->rootdir_sect, mydata->rootdir_sect * SECTOR_SIZE, rootdir_size, mydata->data_begin);
		...
		for (i = 0; i < DIRENTSPERBLOCK; i++) {
				...
				if ((dentptr->attr & ATTR_VFAT) && (dentptr->name[0] & LAST_LONG_ENTRY_MASK)) {
					...
					if (dols == LS_ROOT) {
						...
						if (doit) {
							...
							{
								proc(l_name, dirc == ' ' ? FAT2CPU32(dentptr->size) : 0, dirc, NULL);
							}
						}
						dentptr++;
						continue;
			...
			if (dols == LS_ROOT) {
				...
				if (doit) {
					...
					{
						proc(s_name, dirc == ' ' ? FAT2CPU32(dentptr->size) : 0, dirc, NULL);
					}
				}
				dentptr++;
				continue;
...
{
	return do_fat_read(dir, NULL, 0, LS_YES, proc);
}
