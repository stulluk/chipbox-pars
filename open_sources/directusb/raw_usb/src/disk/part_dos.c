 ...
 */
inline int le32_to_int(unsigned char *le32)
{
...
void print_one_part(dos_partition_t * p, int ext_part_sector, int part_num)
{
 ...
 */
void print_partition_extended(block_dev_desc_t * dev_desc, int ext_part_sector, int relative, int part_num)
{
	unsigned char buffer[DEFAULT_SECTOR_SIZE];
	dos_partition_t *pt;
	int i;
	...
	for (i = 0; i < 4; i++, pt++) {
		if (is_extended(pt->sys_ind)) {
			...
			print_partition_extended(dev_desc, lba_start,
						 ext_part_sector == 0 ? lba_start : relative, part_num);
		}
	}
...
/*  Print a partition that is relative to its Extended partition table
 */
int get_partition_info_extended(block_dev_desc_t * dev_desc, int ext_part_sector,
				       int relative, int part_num, int which_part, disk_partition_t * info)
{
	unsigned char buffer[DEFAULT_SECTOR_SIZE];
	dos_partition_t *pt;
	int i;
