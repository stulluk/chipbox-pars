#include "linuxos.h"

#include "mwsetting.h"
#include "userdefine.h"
#include "cs_app_common.h"
#include "cs_app_main.h"
#include "mvrecfile.h"
#include "csmpr_usb.h"
#include "csmpr_recorder.h"
#include "csmpr_player.h"
#include "ui_common.h"
#include <unistd.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
#include <stdio.h>
#include <pwd.h>
#include <grp.h>

#define LIST_MAX_ITEM			10
#define	MAX_FILE_NAME_LENGTH	255
#define	Mp3_Progress_timer_Max	500
#define	Volume_timer_Max		( 2*1000 )
#define	MEDIA_MUTE_ICON_X		( MV_PIG_RIGHT - 70	)
#define	MEDIA_MUTE_ICON_Y		( MV_PIG_TOP + 20 )
#define VOLUME_X				390//CHEDIT_LIST_LEFT + 30
#define	VOLUME_Y				650
#define	VOLUME_DX				500//450
#define	VOLUME_DY				40
#define	VOLUME_TEXT_X			( VOLUME_X + VOLUME_DX )
#define	VOLUME_TEXT_DX			50
#define	VOLUME_FULL_DX 			( MV_BMP[MVBMP_VOLUME_ICON].bmWidth + VOLUME_DX + VOLUME_TEXT_DX )

typedef struct
{
	U8	Tag_ID[4];
	U8	Title[31];
	U8	Artist[31];
	U8	Album[31];
	U8	Year[5];
	U8	Comment[31];
	U8	Part;
} stMp3Tag;

static 	CSAPP_Applet_t		CSApp_Mplayer_Applets;

static 	stFile_db			stFileDB;
static 	U16					u16Current_index;
static 	U16					u16Current_Focus;
static 	U16					u16Current_Page;
static 	U16					u16Prev_Page;
static 	char 				acCurrent_Dir[MAX_FILE_NAME_LENGTH];
static 	BOOL				File_Delete_Flag = FALSE;
static	BOOL				File_Rename_Flag = FALSE;
static 	BOOL				b8Image_Load_Flag = FALSE;
static 	BOOL				b8Mp3Play_Flag = FALSE;
static 	BITMAP				btBackup_Screen;
static 	BITMAP				btLoading_Image;
static 	tCSDesktopVolume 	PVR_Volume;
static 	U8					u8ThisHDD;

static CSAUD_HANDLE			g_audio_handle = NULL;
static FILE*				aud_file=NULL;
unsigned char  				read_buf[98304];
int 						read_len = 0;
static long					total_read = 0;
static long					File_Size = 0;
stMp3Tag					stMP3_Tag;
static U16					u16Mp3Play_index;
BITMAP						btCap_Volume;

extern CSAUD_HANDLE   		aud_handle;

extern void MV_Calculate_Size(long long llong, char *temp);
int MV_Mp3Play(char *Play_File);
void MV_Mp3Stop(void);
void MV_Mp3Next(void);
void MV_Draw_MPlayer_MenuBar(HDC hdc);
void MV_Draw_MPlayer_FileInfo(HDC hdc, U16 u16Index);
void MV_Draw_MPlayer_StorageInfo(HDC hdc);

/******************************************************************************/
/* 		MP3 Playing Time : (int)filesize / ((bitrate * 1000)/8)               */
/******************************************************************************/
int g_BitrateIndexTable[4][3][16] =
{
	{ // Version 2.5
		{ // Layer I
			0, 32, 48, 56, 64, 80, 96, 112, 128, 144, 160, 176, 192, 224, 256, (-1)
		},
		{ // Layer II
			0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160, (-1)
		},
		{ // Layer III
			0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160, (-1)
		}
	},
	{ // Version reserved
		{0,},{0,},{0,}
	},
	{ // Version 2.0 (ISO / IEC 13818-3)
		{ // Layer I
			0, 32, 48, 56, 64, 80, 96, 112, 128, 144, 160, 176, 192, 224, 256, (-1)
		},
		{ // Layer II
			0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160, (-1)
		},
		{ // Layer III
			0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160, (-1)
		}
	},
	{ // Version 1.0 (ISO / IEC 11172-3)
		{ // Layer I
			0, 32, 64, 96, 128, 160, 192, 224, 256, 288, 320, 352, 384, 416, 448, (-1)
		},
		{ // Layer II
			0, 32, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384, (-1)
		},
		{ // Layer III
			0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, (-1)
		}
	}
};

int g_FrequencyTable[4][4] =
{
	{ // Version 2.5
		11025, 12000, 8000, (-1)
	},
	{ // Version reserved
		(-1),
	},
	{ // Version 2.0
		22050, 24000, 16000, (-1)
	},
	{ // Version 1.0
		44100, 48000, 32000, (-1)
	}
};

char *g_ChannelModeString[] = {
	"Stereo", "Joint steroeo", "Dual stereo", "Single stereo"
};

char *g_LayerString[] = {
	"Layer I", "Layer II", "Layer III"
};

unsigned long MZ_MP3_Time(char *s_MP3_Name)
{
	unsigned long s_Return = 0lu;

	if(s_MP3_Name)
	{
		int s_Handle;

		s_Handle = open(s_MP3_Name, O_RDONLY);

		if(s_Handle >= 0)
		{
			int s_ReadSize;
			unsigned long s_Header = 0lu;
			unsigned char s_Buffer;
			unsigned long s_VersionID = 0lu, s_Layer = 0lu, s_BitrateIndex = 0lu, s_FrequencyIndex = 0lu, s_Padding = 0lu;
			unsigned long s_FrameSize = 0lu, s_FrameCount = 0lu;

			do
			{
				s_ReadSize = read(s_Handle, &s_Buffer, sizeof(unsigned char));
				if(s_ReadSize > 0)
				{
					s_Header = (s_Header << 8) | (unsigned long)(s_Buffer & 0xff);
					if((s_Header & 0xffe00000lu) == 0xffe00000lu)
					{
						s_VersionID = (s_Header >> 19) & 0x03;
						s_Layer = (s_Header >> 17) & 0x03;
						s_BitrateIndex = (s_Header >> 12) & 0x0f;
						s_FrequencyIndex = (s_Header >> 10) & 0x03;
						s_Padding = (s_Header >> 9) & 0x01;
						//s_ChannelMode = (s_Header >> 6) & 0x03;
						if(s_VersionID != 0x01 &&
							s_Layer != 0x00 &&
							s_BitrateIndex != 0x0f && s_BitrateIndex != 0 &&
							s_FrequencyIndex != 0x03  )
						{
							switch((3 - s_Layer))
							{
								case 0: // Layer I
									if ( g_FrequencyTable[s_VersionID][s_FrequencyIndex] == 0 )
									{
										close(s_Handle);
										return 0;
									}
									s_FrameSize = 12000lu * g_BitrateIndexTable[s_VersionID][3 - s_Layer][s_BitrateIndex];
									s_FrameSize /= g_FrequencyTable[s_VersionID][s_FrequencyIndex];
									s_FrameSize = ((s_FrameSize + s_Padding)<<2) - 4;
									break;
								case 1: // Layer II
									if ( g_FrequencyTable[s_VersionID][s_FrequencyIndex] == 0 )
									{
										close(s_Handle);
										return 0;
									}
									s_FrameSize = 144000lu * g_BitrateIndexTable[s_VersionID][3 - s_Layer][s_BitrateIndex];
									s_FrameSize /= g_FrequencyTable[s_VersionID][s_FrequencyIndex];
									s_FrameSize += s_Padding - 4;
								case 2: // Layer III
								default:
									if ( g_FrequencyTable[s_VersionID][s_FrequencyIndex] == 0 )
									{
										close(s_Handle);
										return 0;
									}
									s_FrameSize = 144000lu * g_BitrateIndexTable[s_VersionID][3 - s_Layer][s_BitrateIndex];
									s_FrameSize /= g_FrequencyTable[s_VersionID][s_FrequencyIndex] << (s_VersionID == 0x03 ? 0 : 1);
									s_FrameSize = s_FrameSize + s_Padding - 4;
									break;
							}
							s_Header = 0lu;
							if(s_FrameSize > 0)
							{
								s_FrameCount++;
								lseek(s_Handle, s_FrameSize, SEEK_CUR);
							}
						}  else {
							if ( g_FrequencyTable[s_VersionID][s_FrequencyIndex] == 0 )
							{
								close(s_Handle);
								//printf("s_VersionID Error : %02X\n", s_VersionID);
								return 0;
							}
						}
					}
				}
			}
			while(s_ReadSize > 0);

			close(s_Handle);
			//printf("\n %d / %d ,,, %02X , %02X \n", (s_FrameCount * 1152), g_FrequencyTable[s_VersionID][s_FrequencyIndex], s_VersionID, s_FrequencyIndex);
			if ( g_FrequencyTable[s_VersionID][s_FrequencyIndex] != 0 )
				s_Return = (((s_FrameCount * 1152) / g_FrequencyTable[s_VersionID][s_FrequencyIndex]) >> (s_VersionID == 0x03 ? 0 : 1));
		}
		else
		{
			close(s_Handle);
			//printf("mp3_time.c: MZ_MP3_Time - [ERROR] Can not open mp3 \"%s\" !!!\n", s_MP3_Name);
		}
	}
	else
		printf("mp3_time.c: MZ_MP3_Time - [ERROR] s_MP3_Name is null !!!\n");

	return(s_Return);
}

/******************************************************************************/

BOOL MV_Mp3_Tag_Read(void)
{
	FILE*			aud_temp_file=NULL;
	struct stat		statbuffer;
	char			Temp_str[MAX_FILE_NAME_LENGTH];
	char			Read_Str[MAX_FILE_NAME_LENGTH];

	memset( Temp_str, 0x00, MAX_FILE_NAME_LENGTH);
	memset( &stMP3_Tag, 0x00, sizeof(stMp3Tag));

	sprintf(Temp_str, "%s/%s", acCurrent_Dir, stFileDB.acFileName[u16Current_index]);

	if( stat(Temp_str, &statbuffer ) != 0 )
		printf("STAT ERROR========================\n");
	else
	{
		File_Size = statbuffer.st_size;
	}

	memset( Read_Str, 0x00, MAX_FILE_NAME_LENGTH);
	aud_temp_file = fopen(Temp_str, "r");
	fseek(aud_temp_file, File_Size - 128, SEEK_SET);
	fread(Read_Str, 1, 128, aud_temp_file);
	fclose(aud_temp_file);

	strncpy(stMP3_Tag.Tag_ID, Read_Str, 3);
	strncpy(stMP3_Tag.Title, &Read_Str[3], 30);
	strncpy(stMP3_Tag.Artist, &Read_Str[33], 30);
	strncpy(stMP3_Tag.Album, &Read_Str[63], 30);
	strncpy(stMP3_Tag.Year, &Read_Str[93], 4);
	strncpy(stMP3_Tag.Comment, &Read_Str[97], 30);
	stMP3_Tag.Part = Read_Str[127];

#if 0
	printf("\n============================\n");
	printf("Tag : %s\n", stMP3_Tag.Tag_ID);
	printf("Title : %s\n", stMP3_Tag.Title);
	printf("Artist : %s\n", stMP3_Tag.Artist);
	printf("Album : %s\n", stMP3_Tag.Album);
	printf("Year : %s\n", stMP3_Tag.Year);
	printf("Comment : %s\n", stMP3_Tag.Comment);
	printf("Part : %c\n", stMP3_Tag.Part);
	printf("============================\n");
#endif

	if ( strcmp(stMP3_Tag.Tag_ID , "TAG" ) != 0 )
		return FALSE;
	else
		return TRUE;
}

void MV_Mp3Stop(void)
{
	tCS_DB_ServiceManageData	item_data;
	HWND						hwnd;
	HDC 						hdc;

	b8Mp3Play_Flag = FALSE;
	memset( &stMP3_Tag, 0x00, sizeof(stMp3Tag));

	hwnd = GetActiveWindow();
	CS_DB_GetCurrentList_ServiceData(&item_data, CS_DB_GetCurrentService_OrderIndex());

	hdc = BeginPaint(hwnd);
	MV_SetBrushColor( hdc, MVAPP_TRANSPARENTS_COLOR );
	MV_FillBox( hdc, ScalerWidthPixel(MV_PIG_LEFT + 10), ScalerHeigthPixel(MV_PIG_TOP + 10), ScalerWidthPixel(MV_PIG_DX - 20), ScalerHeigthPixel(MV_PIG_DY - 20) );
	EndPaint(hwnd,hdc);

#if 0
	CS_AV_ProgramStop();
#else
	CSAUD_Stop(g_audio_handle);
	CSAUD_PFMClose(g_audio_handle);
	CSAUD_Close(g_audio_handle);
#endif
	//printf("g_audio_handle STOP\n");
	aud_handle = CSAUD_Open(AUD_DEV_0);

	if( aud_handle == NULL )
	{
		printf("CSVID_Open error\n");
		return;
	}

	CS_MW_PlayServiceByIdx(item_data.Service_Index, TRUE);
}

void MV_Mp3Next(void)
{
	U16			i;
	BOOL		b8Find = FALSE;
	char		Temp_str[MAX_FILE_NAME_LENGTH];
	HWND		hwnd;
	HDC 		hdc;

	hwnd = GetActiveWindow();

	CSAUD_Stop(g_audio_handle);
	CSAUD_PFMClose(g_audio_handle);
	CSAUD_Close(g_audio_handle);

	memset( Temp_str, 0x00, MAX_FILE_NAME_LENGTH);

	for ( i = u16Current_index + 1 ; i < stFileDB.u16file_Count ; i++ )
	{
		if ( MV_Check_Filetype(stFileDB.acFileExt[i]) == MVAPP_FILE_MUSIC )
		{
			b8Find = TRUE;
			u16Mp3Play_index = u16Current_index = i;
			sprintf(Temp_str, "%s/%s", acCurrent_Dir, stFileDB.acFileName[u16Current_index]);
			//printf("\n==== First : %s \n", Temp_str);
			break;
		}
	}

	if ( b8Find == FALSE )
	{
		for ( i = 0 ; i < stFileDB.u16file_Count ; i++ )
		{
			if ( MV_Check_Filetype(stFileDB.acFileExt[i]) == MVAPP_FILE_MUSIC )
			{
				b8Find = TRUE;
				u16Mp3Play_index = u16Current_index = i;
				sprintf(Temp_str, "%s/%s", acCurrent_Dir, stFileDB.acFileName[u16Current_index]);
				//printf("\n==== Second : %s \n", Temp_str);
				break;
			}
		}
	}

	u16Current_Focus = get_focus_line(&u16Current_Page, u16Current_index, LIST_MAX_ITEM);

	MV_Mp3Play(Temp_str);

	hdc = BeginPaint(hwnd);
	MV_Draw_MPlayer_MenuBar(hdc);
	MV_Draw_MPlayer_FileInfo(hdc, u16Current_index);
	MV_Draw_MPlayer_StorageInfo(hdc);
	EndPaint(hwnd,hdc);
}

void audiocallbackE(CSAUD_HANDLE handle)
{
	unsigned int bufsize = 0;

	//printf(" empty audio call back run!\n");

	if(CSAPI_SUCCEED == CSAUD_WritePFMData(handle, read_buf, read_len))
	{
		if ((read_len=fread(read_buf,1,4096,aud_file))!=4096)
		{
			fseek(aud_file,0L, SEEK_SET);
			printf("3 read_len = %d ,===no data \n",read_len);
			//printf("STOPED UP ========\n");
			MV_Mp3Next();
			return;
		} else
			total_read += read_len;
		CSAUD_GetPFMBufferSize(g_audio_handle, &bufsize);
		//printf("3 bufsize = %d\n",bufsize);
	} else {
		//printf("STOPED DOWN ========\n");
		MV_Mp3Stop();
	}
}

void audiocallbackF(CSAUD_HANDLE handle)
{
	printf(" full audio call back run! %d\n", (int)handle); // JUST Warning
}

int MV_Mp3Play(char *Play_File)
{
	unsigned int bufsize = 0;
	unsigned int datasize = 50;
	int read_len = 0;
	total_read = 0;
	File_Size = 1;

	u16Mp3Play_index = u16Current_index;

	MV_Mp3_Tag_Read();

	if ( b8Mp3Play_Flag == FALSE )
	{
		CS_AV_ProgramStop();
		CS_AV_Play_IFrame(RADIO_BACK);

		b8Mp3Play_Flag = TRUE;
	}

	if(NULL == (g_audio_handle=CSAUD_Open(AUD_DEV_1)))
	{
		printf("open audio error\n");
		return CSAPI_FAILED;
	}
	else
	{
		printf("open audio ok\n");
	}

	CSAUD_Init(g_audio_handle);
	CSAUD_SetInputMode(g_audio_handle,AUD_INPUT_BLOCK);
	CSAUD_SetCodecType(g_audio_handle,AUD_STREAM_TYPE_MPA);
	//CSAUD_SetVolume(g_audio_handle, &vol);
	CSAUD_SetOutputDevice( g_audio_handle, AUD_OUTPUT_I2S_SPDIFPCM );  //DREJO 17.01.2013
	CSAUD_Play(g_audio_handle);

	CSAUD_PFMOpen(g_audio_handle);
	CSAUD_GetPFMBufferSize(g_audio_handle, &bufsize);
	printf("1 bufsize = %d\n",bufsize);

	aud_file = fopen(Play_File,"rb");
	if(aud_file == NULL)
	{
		printf("======>file open failed\n");
		return CSAPI_FAILED;
	}


	if ((read_len=fread(read_buf,1,4096,aud_file))!=4096)
	{
		fseek(aud_file,0L, SEEK_SET);
		printf("1 read_len = %d ,===no data \n",read_len);
	} else
		total_read += read_len;

	CSAUD_SetNotifyPFMDataEmpty(g_audio_handle, &datasize, (void *)audiocallbackE, 1);
	CSAUD_SetNotifyPFMDataFull(g_audio_handle, &datasize, (void *)audiocallbackF, 1);

	return CSAPI_SUCCEED;
}

void MV_Mp3Progress(HDC hdc)
{
	U16				LevelValue;
	RECT			TmpRect;

	//printf("\n=== Toatal : %ld / Filesize : %ld = %ld\n", ( total_read * 100 ) , File_Size, ( total_read * 100 ) / File_Size);
	LevelValue = ( total_read * 100 ) / File_Size;

	TmpRect.left = MV_PIG_LEFT ;
	TmpRect.right = TmpRect.left + MV_PIG_DX - 60;
	TmpRect.top = MV_PIG_TOP + MV_PIG_DY - 30;
	TmpRect.bottom = TmpRect.top + 14;
	MV_Draw_LevelBar(hdc, &TmpRect, LevelValue, EN_TTEM_PROGRESS_NO_IMG_MP3);
}

void MV_MPlayer_Sort_Files(void)
{
	int 			i = 0, j = 0;
	U16				Dir_count = 0;
	U16				File_count = 0;
	stFile_db_temp	stSort_Temp;
	stFile_db		stFileDB_Dir;
	stFile_db		stFileDB_File;
	struct stat		statbuffer;
	char			TmpStr[MAX_FILE_NAME_LENGTH];

	memset ( &stSort_Temp, 0x00, sizeof(stFile_db_temp));
	memset ( &stFileDB_Dir, 0x00, sizeof(stFile_db));
	memset ( &stFileDB_File, 0x00, sizeof(stFile_db));

	for ( i = 0 ; i < stFileDB.u16file_Count ; i++ )
	{
		sprintf(TmpStr, "%s/%s", acCurrent_Dir, stFileDB.acFileName[i]);

		if( stat(TmpStr, &statbuffer ) != 0 )
			printf("STAT ERROR========================\n");

		if( S_ISDIR(statbuffer.st_mode) )
		{
			strcpy(stFileDB_Dir.acFileName[Dir_count], stFileDB.acFileName[i]);
			strcpy(stFileDB_Dir.acFileExt[Dir_count], stFileDB.acFileExt[i]);
			Dir_count++;
			//printf("DIRECTORY : %d : %s <- %s \n", Dir_count, stFileDB_Dir.acFileName[Dir_count-1] , stFileDB.acFileName[i]);
		} else {
			strcpy(stFileDB_File.acFileName[File_count], stFileDB.acFileName[i]);
			strcpy(stFileDB_File.acFileExt[File_count], stFileDB.acFileExt[i]);
			File_count++;
			//printf("FILE : %d : %s <- %s\n", File_count, stFileDB_File.acFileName[File_count-1] , stFileDB.acFileName[i]);
		}
	}
	stFileDB_Dir.u16file_Count = Dir_count;
	stFileDB_File.u16file_Count = File_count;

	i = 0 ;

	while ( i < stFileDB_Dir.u16file_Count )
	{
		for ( j = i ; j < stFileDB_Dir.u16file_Count - 1 ; j++ )
		{
			if ( strcasecmp( stFileDB_Dir.acFileName[j], stFileDB_Dir.acFileName[j+1]) > 0 )
			{
				//printf("=== %s => %s \n", stFileDB_Dir.acFileName[j], stFileDB_Dir.acFileName[j+1]);
				strcpy(stSort_Temp.acFileName, stFileDB_Dir.acFileName[j]);
				strcpy(stSort_Temp.acFileExt, stFileDB_Dir.acFileExt[j]);

				strcpy(stFileDB_Dir.acFileName[j], stFileDB_Dir.acFileName[j+1]);
				strcpy(stFileDB_Dir.acFileExt[j], stFileDB_Dir.acFileExt[j+1]);

				strcpy(stFileDB_Dir.acFileName[j+1], stSort_Temp.acFileName);
				strcpy(stFileDB_Dir.acFileExt[j+1], stSort_Temp.acFileExt);
			}
		}
		i++;
	}

	i = 0;

	while ( i < stFileDB_File.u16file_Count )
	{
		for ( j = i ; j < stFileDB_File.u16file_Count - 1 ; j++ )
		{
			if ( strcasecmp( stFileDB_File.acFileName[j], stFileDB_File.acFileName[j+1]) > 0 )
			{
				strcpy(stSort_Temp.acFileName, stFileDB_File.acFileName[j]);
				strcpy(stSort_Temp.acFileExt, stFileDB_File.acFileExt[j]);

				strcpy(stFileDB_File.acFileName[j], stFileDB_File.acFileName[j+1]);
				strcpy(stFileDB_File.acFileExt[j], stFileDB_File.acFileExt[j+1]);

				strcpy(stFileDB_File.acFileName[j+1], stSort_Temp.acFileName);
				strcpy(stFileDB_File.acFileExt[j+1], stSort_Temp.acFileExt);
			}
		}
		i++;
	}

	//printf("\n=== DIR : %d === FILE : %d ===\n", stFileDB_Dir.u16file_Count, stFileDB_File.u16file_Count);

	memset ( &stFileDB, 0x00, sizeof(stFile_db));

	for ( i = 0; i < stFileDB_Dir.u16file_Count ; i++ )
	{
		strcpy(stFileDB.acFileName[i] , stFileDB_Dir.acFileName[i]);
		strcpy(stFileDB.acFileExt[i] , stFileDB_Dir.acFileExt[i]);
		//printf("DIRECTORY : %d : %s -> %s \n", i, stFileDB_Dir.acFileName[i] , stFileDB.acFileName[i]);
	}

	for ( i = 0; i < stFileDB_File.u16file_Count ; i++ )
	{
		strcpy(stFileDB.acFileName[stFileDB_Dir.u16file_Count + i] , stFileDB_File.acFileName[i]);
		strcpy(stFileDB.acFileExt[stFileDB_Dir.u16file_Count + i] , stFileDB_File.acFileExt[i]);
		//printf("FILE : %d : %s -> %s \n", i, stFileDB_File.acFileName[i] , stFileDB.acFileName[stFileDB_Dir.u16file_Count + i]);
	}

	stFileDB.u16file_Count = stFileDB_Dir.u16file_Count + stFileDB_File.u16file_Count;
}

MV_File_Return MV_MPlayer_Load_fileData(char *Read_Dir)
{
	DIR				*pDir = NULL;
	struct dirent 	*pDirEnt;
	struct stat		statbuffer;
	U16				i;
	char			TmpStr[MAX_FILE_NAME_LENGTH];

	memset(&stFileDB, 0x00, sizeof(stFile_db));

	if( (pDir=opendir( Read_Dir )) == NULL )
	{
		closedir( pDir );
		printf( "[UsbCon_Mount]:GetUsbDevFileName(): Failed to open dir.\n" );
		return FILE_READ_FAIL;
	}

	if ( strcmp(Read_Dir, stPartition_data[u8ThisHDD].acPartition_Dir) == 0 )
	{
		while( (pDirEnt=readdir( pDir )) != NULL )
		{
			sprintf(TmpStr, "%s/%s", acCurrent_Dir, pDirEnt->d_name);
			if( stat(TmpStr, &statbuffer ) != 0 )
				printf("STAT ERROR========================\n");

			if( strcmp( pDirEnt->d_name, "." ) != 0 && strcmp( pDirEnt->d_name, "..") != 0 )
			{
				strcpy(stFileDB.acFileName[stFileDB.u16file_Count], pDirEnt->d_name);

				for ( i = strlen(pDirEnt->d_name) ; i > 0 ; i-- )
				{
					if ( pDirEnt->d_name[i] == '.' )
						break;
				}

				if( S_ISDIR(statbuffer.st_mode) )
					memset(stFileDB.acFileExt[stFileDB.u16file_Count], 0x00, 10);
				else if ( i < strlen(pDirEnt->d_name) - 1 && i > 1 )
				{
					//printf("==== %d , %s , %c , %d ====\n", i, pDirEnt->d_name, pDirEnt->d_name[i+1], strlen(pDirEnt->d_name));
					strncpy(stFileDB.acFileExt[stFileDB.u16file_Count], &pDirEnt->d_name[i+1], strlen(pDirEnt->d_name) - i);
					//printf("==== %s : EXT : %d : %s =======\n", stFileDB.acFileName[stFileDB.u16file_Count], i, stFileDB.acFileExt[stFileDB.u16file_Count]);
				}

				stFileDB.u16file_Count++;
			}
		}
	} else {
		while( (pDirEnt=readdir( pDir )) != NULL )
		{
			sprintf(TmpStr, "%s/%s", acCurrent_Dir, pDirEnt->d_name);
			if( stat(TmpStr, &statbuffer ) != 0 )
				printf("STAT ERROR========================\n");

			if( strcmp( pDirEnt->d_name, "." ) != 0 )
			{
				strcpy(stFileDB.acFileName[stFileDB.u16file_Count], pDirEnt->d_name);

				if( strcmp( pDirEnt->d_name, ".." ) != 0 )
				{
					for ( i = strlen(pDirEnt->d_name) ; i > 0 ; i-- )
					{
						if ( pDirEnt->d_name[i] == '.' )
							break;
					}

					if( S_ISDIR(statbuffer.st_mode) )
						memset(stFileDB.acFileExt[stFileDB.u16file_Count], 0x00, 10);
					else if ( i < strlen(pDirEnt->d_name) - 1 && i > 1 )
					{
						//printf("==== %d , %s , %c , %d ====\n", i, pDirEnt->d_name, pDirEnt->d_name[i+1], strlen(pDirEnt->d_name));
						strncpy(stFileDB.acFileExt[stFileDB.u16file_Count], &pDirEnt->d_name[i+1], strlen(pDirEnt->d_name) - i);
						//printf("==== %s : EXT : %d : %s =======\n", stFileDB.acFileName[stFileDB.u16file_Count], i, stFileDB.acFileExt[stFileDB.u16file_Count]);
					}
				}

				stFileDB.u16file_Count++;
			}
		}
	}

	MV_MPlayer_Sort_Files();

	closedir( pDir );
	return FILE_OK;
}

void MV_Draw_MPlayer_MenuTitle(HDC hdc)
{
	RECT 	TmpRect;
	char	TmpStr[10];

	MV_SetBrushColor( hdc, MVAPP_BLACK_COLOR_ALPHA );
	MV_FillBox( hdc, ScalerWidthPixel(CHEDIT_LIST_LEFT - 10),ScalerHeigthPixel( MV_INSTALL_MENU_Y - 10), ScalerWidthPixel(CHEDIT_LIST_DX + 20 ),ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H) );

	SetTextColor(hdc,MV_BAR_UNFOCUS_CHAR_COLOR);
	SetBkMode(hdc,BM_TRANSPARENT);

	TmpRect.left	= ScalerWidthPixel(CHEDIT_LIST_LEFT + 20);
	TmpRect.right	= TmpRect.left + (CHEDIT_LIST_DX - 30);
	TmpRect.top		= ScalerHeigthPixel(MV_INSTALL_MENU_Y - 7);
	TmpRect.bottom	= TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

	sprintf(TmpStr, "%d/%d", u16Current_index + 1 , stFileDB.u16file_Count);
	CS_MW_DrawText(hdc, CS_MW_LoadStringByIdx(CSAPP_STR_NAME), -1, &TmpRect, DT_LEFT);
	CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_RIGHT);
}

void MV_Draw_MPlayer_Partition_Info(HDC hdc)
{
	RECT 	TmpRect;
	char	TmpStr[50];
	char 	TempStr[10];

	MV_SetBrushColor( hdc, MVAPP_BLACK_COLOR_ALPHA );
	MV_FillBox( hdc, ScalerWidthPixel(MV_PIG_LEFT), ScalerHeigthPixel( MV_INSTALL_MENU_Y - 10 ), ScalerWidthPixel(MV_PIG_LIST_DX),ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H) );

	SetTextColor(hdc,MV_BAR_UNFOCUS_CHAR_COLOR);
	SetBkMode(hdc,BM_TRANSPARENT);

	TmpRect.left	= MV_PIG_LEFT + 20;
	TmpRect.right	= TmpRect.left + (MV_PIG_LIST_DX - 20);
	TmpRect.top		= MV_INSTALL_MENU_Y - 7;
	TmpRect.bottom	= TmpRect.top + MV_INSTALL_MENU_BAR_H;

	//printf("=== %d , %d , %d ,%d ===\n", TmpRect.left, TmpRect.right, TmpRect.top, TmpRect.bottom);
	strncpy(TempStr, &stPartition_data[u8ThisHDD].acPartition_Dir[strlen(stPartition_data[u8ThisHDD].acPartition_Dir) - 5], 5);
	sprintf(TmpStr, "%s : %s", CS_MW_LoadStringByIdx(CSAPP_STR_PARTITION), TempStr);
	//printf("=== %d , %d , %d ,%d , %s ===\n", TmpRect.left, TmpRect.right, TmpRect.top, TmpRect.bottom, TmpStr);
	CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);
}


void MV_Draw_MPlayer_FileInfo(HDC hdc, U16 u16Index)
{
	RECT			TmpRect;
	struct stat		statbuffer;
	char			TmpStr[MAX_FILE_NAME_LENGTH];
	char			TmpStr2[MAX_FILE_NAME_LENGTH];
	struct 			tm *tm_ptr;

	sprintf(TmpStr, "%s/%s", acCurrent_Dir, stFileDB.acFileName[u16Index]);
	//CS_PVR_Player_Start(TmpStr);
	if( stat(TmpStr, &statbuffer ) != 0 )
		printf("STAT ERROR========================\n");

	MV_SetBrushColor( hdc, MVAPP_BLACK_COLOR_ALPHA );
	MV_FillBox( hdc, ScalerWidthPixel(MV_PIG_LIST_LEFT),ScalerHeigthPixel( MV_PIG_LIST_TOP + MV_PIG_LIST_DY + 10 ), ScalerWidthPixel(MV_PIG_LIST_DX),ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H * 3) );

	SetTextColor(hdc,MV_BAR_UNFOCUS_CHAR_COLOR);
	SetBkMode(hdc,BM_TRANSPARENT);

	TmpRect.left	=ScalerWidthPixel(MV_PIG_LIST_LEFT + 30);
	TmpRect.right	=TmpRect.left + MV_PIG_LIST_DX - 80;
	TmpRect.top		=ScalerHeigthPixel(MV_PIG_LIST_TOP + MV_PIG_LIST_DY + 10) + 2;
	TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

	sprintf(TmpStr, "%s : %s", CS_MW_LoadStringByIdx(CSAPP_STR_NAME), stFileDB.acFileName[u16Index]);
	CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);

	TmpRect.top		=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
	TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

	MV_Calculate_Size( ( statbuffer.st_size / 1024 ), TmpStr2);
	sprintf(TmpStr, "%s : %s [ %lld Byte ]", CS_MW_LoadStringByIdx(CSAPP_STR_SIZE), TmpStr2, statbuffer.st_size);
	CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);

	TmpRect.top		=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
	TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
	tm_ptr = gmtime(&statbuffer.st_mtime);

	sprintf(TmpStr, "%s : %d-%d-%d %d:%d:%d",
					CS_MW_LoadStringByIdx(CSAPP_STR_TIME),
					tm_ptr->tm_year+1900,
                    tm_ptr->tm_mon+1,
                    tm_ptr->tm_mday,
                    tm_ptr->tm_hour,
                    tm_ptr->tm_min,
                    tm_ptr->tm_sec);

	CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);
	//CS_PVR_Player_Stop();
}


void MV_Draw_MPlayer_StorageInfo(HDC hdc)
{
	RECT			TmpRect;
	char			TmpStr[MAX_FILE_NAME_LENGTH];
	char			Temp_Str[MAX_FILE_NAME_LENGTH];
	char			File_Str[MAX_FILE_NAME_LENGTH];
	long			Duration_Time = 0;
	U16				Duration_Min = 0;
	U16				Duration_Sec = 0;
	//struct tm 		*tm_ptr;

	memset (TmpStr, 0, sizeof(char) * MAX_FILE_NAME_LENGTH);
	memset (Temp_Str, 0, sizeof(char) * MAX_FILE_NAME_LENGTH);
	memset (File_Str, 0, sizeof(char) * MAX_FILE_NAME_LENGTH);

	MV_SetBrushColor( hdc, MVAPP_BLACK_COLOR_ALPHA );
	MV_FillBox( hdc, ScalerWidthPixel(MV_PIG_LEFT),ScalerHeigthPixel( MV_PIG_TOP + MV_PIG_DY + 10 ), ScalerWidthPixel(MV_PIG_DX),ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H * 5) );

	SetTextColor(hdc,MV_BAR_UNFOCUS_CHAR_COLOR);
	SetBkMode(hdc,BM_TRANSPARENT);

	TmpRect.left	=ScalerWidthPixel(MV_PIG_LEFT + 30);
	TmpRect.right	=TmpRect.left + MV_PIG_DX - 80;
	TmpRect.top		=ScalerHeigthPixel(MV_PIG_TOP + MV_PIG_DY + 10) + 2;
	TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

	if ( strlen( stMP3_Tag.Tag_ID ) > 0 )
	{
		if ( strcmp( stMP3_Tag.Tag_ID , "TAG" ) != 0 )
		{
			sprintf(TmpStr, "%s : %s", CS_MW_LoadStringByIdx(CSAPP_STR_TITLE), CS_MW_LoadStringByIdx(CSAPP_STR_NONE));
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);

			TmpRect.top		=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
			TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

			sprintf(TmpStr, "%s : %s", CS_MW_LoadStringByIdx(CSAPP_STR_ARTIST), CS_MW_LoadStringByIdx(CSAPP_STR_NONE));
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);

			TmpRect.top		=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
			TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

			sprintf(TmpStr, "%s : %s", CS_MW_LoadStringByIdx(CSAPP_STR_ALBUM), CS_MW_LoadStringByIdx(CSAPP_STR_NONE));
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);

			TmpRect.top		=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
			TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

			sprintf(TmpStr, "%s : %s",  CS_MW_LoadStringByIdx(CSAPP_STR_YEAR), CS_MW_LoadStringByIdx(CSAPP_STR_NONE));
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);

			TmpRect.top		=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
			TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

			sprintf(Temp_Str, "%s/%s", acCurrent_Dir, stFileDB.acFileName[u16Mp3Play_index]);
			Duration_Time = MZ_MP3_Time(Temp_Str);
			Duration_Min = Duration_Time/60;
			Duration_Sec = Duration_Time%60;
			sprintf(TmpStr, "Duration : %02d : %02d", Duration_Min, Duration_Sec);
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);
		} else {
			sprintf(TmpStr, "Title : %s", stMP3_Tag.Title);
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);

			TmpRect.top		=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
			TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

			sprintf(TmpStr, "Artist : %s", stMP3_Tag.Artist);
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);

			TmpRect.top		=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
			TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

			sprintf(TmpStr, "Album : %s", stMP3_Tag.Album);
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);

			TmpRect.top		=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
			TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

			sprintf(TmpStr, "Year : %s", stMP3_Tag.Year);
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);

			TmpRect.top		=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);
			TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H);

			sprintf(Temp_Str, "%s/%s", acCurrent_Dir, stFileDB.acFileName[u16Mp3Play_index]);
			Duration_Time = MZ_MP3_Time(Temp_Str);
			Duration_Min = Duration_Time/60;
			Duration_Sec = Duration_Time%60;
			sprintf(TmpStr, "Duration : %02d : %02d", Duration_Min, Duration_Sec);
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_LEFT);
		}
	}
}

void MV_Draw_MPlayerMenuBar(HDC hdc, int esItem, U8 u8Kind)
{
	int 			y_gap = MV_INSTALL_MENU_Y + MV_INSTALL_MENU_BAR_H * ( esItem + 1 );
	RECT			TmpRect;
	struct stat		statbuffer;
	char			TmpStr[MAX_FILE_NAME_LENGTH];

	SetTextColor(hdc,MV_BAR_UNFOCUS_CHAR_COLOR);
	SetBkMode(hdc,BM_TRANSPARENT);

	if( u8Kind == FOCUS ) {
		FillBoxWithBitmap (hdc, ScalerWidthPixel(CHEDIT_LIST_LEFT), ScalerHeigthPixel(y_gap), ScalerWidthPixel(CHEDIT_LIST_DX - 20), ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H), &MV_BMP[MVBMP_CHLIST_SELBAR]);
	} else {
		if ( esItem % 2 == 0 )
		{
			MV_SetBrushColor( hdc, MVAPP_BLACK_COLOR_ALPHA );
			MV_FillBox( hdc, ScalerWidthPixel(CHEDIT_LIST_LEFT),ScalerHeigthPixel( y_gap ), ScalerWidthPixel(CHEDIT_LIST_DX - 20),ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H) );
		} else {
			MV_SetBrushColor( hdc, MVAPP_DARKBLUE_COLOR );
			MV_FillBox( hdc, ScalerWidthPixel(CHEDIT_LIST_LEFT),ScalerHeigthPixel( y_gap ), ScalerWidthPixel(CHEDIT_LIST_DX - 20),ScalerHeigthPixel(MV_INSTALL_MENU_BAR_H) );
		}
	}

	if ( ( esItem + ( u16Current_Page * LIST_MAX_ITEM) ) < stFileDB.u16file_Count )
	{
		TmpRect.left	=ScalerWidthPixel(MV_INSTALL_MENU_X) + 6;
		TmpRect.right	=TmpRect.left + (CHEDIT_LIST_DX - 210);
		TmpRect.top		=ScalerHeigthPixel(y_gap+4);
		TmpRect.bottom	=TmpRect.top + ScalerHeigthPixel(MV_INSTALL_MENU_HEIGHT2);

		sprintf(TmpStr, "%s/%s", acCurrent_Dir, stFileDB.acFileName[esItem + ( u16Current_Page * LIST_MAX_ITEM)]);
		if( stat(TmpStr, &statbuffer ) != 0 )
			printf("STAT ERROR========================\n");

		if( S_ISDIR(statbuffer.st_mode) )
		{
			if ( strcmp ( stFileDB.acFileName[esItem + ( u16Current_Page * LIST_MAX_ITEM)], ".." ) != 0 )
				FillBoxWithBitmap (hdc, ScalerWidthPixel(TmpRect.left - MV_BMP[MVBMP_DIR_FOLDER].bmWidth ), ScalerHeigthPixel(y_gap + 2), ScalerWidthPixel(MV_BMP[MVBMP_DIR_FOLDER].bmWidth), ScalerHeigthPixel(MV_BMP[MVBMP_DIR_FOLDER].bmHeight), &MV_BMP[MVBMP_DIR_FOLDER]);
		}
		else
		{
			switch( MV_Check_Filetype(stFileDB.acFileExt[esItem + ( u16Current_Page * LIST_MAX_ITEM)] ) )
			{
				case MVAPP_FILE_TS:
					FillBoxWithBitmap (hdc, ScalerWidthPixel(TmpRect.left - MV_BMP[MVBMP_TS_FILE].bmWidth ), ScalerHeigthPixel(y_gap + 2), ScalerWidthPixel(MV_BMP[MVBMP_TS_FILE].bmWidth), ScalerHeigthPixel(MV_BMP[MVBMP_TS_FILE].bmHeight), &MV_BMP[MVBMP_TS_FILE]);
					break;
				case MVAPP_FILE_MOVIE:
					FillBoxWithBitmap (hdc, ScalerWidthPixel(TmpRect.left - MV_BMP[MVBMP_MOVIE_FILE].bmWidth ), ScalerHeigthPixel(y_gap + 2), ScalerWidthPixel(MV_BMP[MVBMP_MOVIE_FILE].bmWidth), ScalerHeigthPixel(MV_BMP[MVBMP_MOVIE_FILE].bmHeight), &MV_BMP[MVBMP_MOVIE_FILE]);
					break;
				case MVAPP_FILE_MUSIC:
					FillBoxWithBitmap (hdc, ScalerWidthPixel(TmpRect.left - MV_BMP[MVBMP_MUSIC_FILE].bmWidth ), ScalerHeigthPixel(y_gap + 2), ScalerWidthPixel(MV_BMP[MVBMP_MUSIC_FILE].bmWidth), ScalerHeigthPixel(MV_BMP[MVBMP_MUSIC_FILE].bmHeight), &MV_BMP[MVBMP_MUSIC_FILE]);
					break;
				case MVAPP_FILE_PIC:
					FillBoxWithBitmap (hdc, ScalerWidthPixel(TmpRect.left - MV_BMP[MVBMP_IMAGE_FILE].bmWidth ), ScalerHeigthPixel(y_gap + 2), ScalerWidthPixel(MV_BMP[MVBMP_IMAGE_FILE].bmWidth), ScalerHeigthPixel(MV_BMP[MVBMP_IMAGE_FILE].bmHeight), &MV_BMP[MVBMP_IMAGE_FILE]);
					break;
				case MVAPP_FILE_TEXT:
					FillBoxWithBitmap (hdc, ScalerWidthPixel(TmpRect.left - MV_BMP[MVBMP_TEXT_FILE].bmWidth ), ScalerHeigthPixel(y_gap + 2), ScalerWidthPixel(MV_BMP[MVBMP_TEXT_FILE].bmWidth), ScalerHeigthPixel(MV_BMP[MVBMP_TEXT_FILE].bmHeight), &MV_BMP[MVBMP_TEXT_FILE]);
					break;
				default :
					FillBoxWithBitmap (hdc, ScalerWidthPixel(TmpRect.left - MV_BMP[MVBMP_NORMAL_FILE].bmWidth ), ScalerHeigthPixel(y_gap + 2), ScalerWidthPixel(MV_BMP[MVBMP_NORMAL_FILE].bmWidth), ScalerHeigthPixel(MV_BMP[MVBMP_NORMAL_FILE].bmHeight), &MV_BMP[MVBMP_NORMAL_FILE]);
					break;
			}
		}

		TmpRect.left	=ScalerWidthPixel(MV_INSTALL_MENU_X) + 10;
		CS_MW_DrawText(hdc, stFileDB.acFileName[esItem + ( u16Current_Page * LIST_MAX_ITEM)], -1, &TmpRect, DT_LEFT);

		if( !(S_ISDIR(statbuffer.st_mode)) )
		{
			TmpRect.left	=TmpRect.right;
			TmpRect.right	=TmpRect.left + 160;

			MV_Calculate_Size( ( statbuffer.st_size / 1024 ), TmpStr);
			//sprintf(TmpStr, "%d", statbuffer.st_size);
			CS_MW_DrawText(hdc, TmpStr, -1, &TmpRect, DT_RIGHT);
		}
	}

	if ( u8Kind == FOCUS )
	{
		TmpRect.top = CHEDIT_SCROLL_TOP - 6;
		TmpRect.left = CHEDIT_SCROLL_LEFT;
		TmpRect.right = CHEDIT_SCROLL_RIGHT;
		TmpRect.bottom = CHEDIT_SCROLL_BOTTOM + 6;
		MV_Draw_ScrollBar(hdc, TmpRect, u16Current_index, stFileDB.u16file_Count - 1, EN_ITEM_CHANNEL_LIST, MV_VERTICAL);
	}
}

void MV_Draw_MPlayer_MenuBar(HDC hdc)
{
	U16 	i = 0;

	for( i = 0 ; i < LIST_MAX_ITEM ; i++ )
	{
		if ( i == u16Current_Focus )
			MV_Draw_MPlayerMenuBar(hdc, i, FOCUS);
		else
			MV_Draw_MPlayerMenuBar(hdc, i, UNFOCUS);
	}
}

void MV_MPlayer_Under_Dir(void)
{
	int		i = 0;

	for ( i = strlen(acCurrent_Dir) ; i > 0 ; i-- )
	{
		if ( acCurrent_Dir[i] == '/' )
		{
			acCurrent_Dir[i] = 0x00;
			break;
		} else {
			acCurrent_Dir[i] = 0x00;
		}
	}
}

void Mv_Image_Viewer(HWND hwnd, stFile_db *stFileDB)
{
	char		Temp_Str[256];
	HDC			hdc;
	U16			Start_X;
	U16			Start_Y;

	b8Image_Load_Flag = TRUE;
	memset ( Temp_Str, 0x00, 256 );

	hdc = BeginPaint(hwnd);
	sprintf(Temp_Str, "%s/%s", acCurrent_Dir, stFileDB->acFileName[u16Current_index]);

	MV_GetBitmapFromDC (hdc, ScalerWidthPixel(0),ScalerHeigthPixel(0), ScalerWidthPixel(CSAPP_OSD_MAX_WIDTH), ScalerHeigthPixel(CSAPP_OSD_MAX_HEIGHT), &btBackup_Screen);
	MV_BitmapFromFile(HDC_SCREEN, &btLoading_Image, Temp_Str);

	MV_SetBrushColor( hdc, MVAPP_BLACK_COLOR_ALPHA );
	MV_FillBox( hdc, ScalerWidthPixel(0),ScalerHeigthPixel(0), ScalerWidthPixel(CSAPP_OSD_MAX_WIDTH),ScalerHeigthPixel(CSAPP_OSD_MAX_HEIGHT) );

	printf("\n Width = %d, Height = %d ========>>> \n", btLoading_Image.bmWidth, btLoading_Image.bmHeight);

	if ( btLoading_Image.bmWidth < CSAPP_OSD_MAX_WIDTH )
	{
		if ( btLoading_Image.bmHeight < CSAPP_OSD_MAX_HEIGHT )
		{
			Start_X = ( CSAPP_OSD_MAX_WIDTH - btLoading_Image.bmWidth ) / 2;
			Start_Y = ( CSAPP_OSD_MAX_HEIGHT - btLoading_Image.bmHeight ) / 2;
			MV_FillBoxWithBitmap (hdc, ScalerWidthPixel(Start_X), ScalerHeigthPixel(Start_Y), ScalerWidthPixel(btLoading_Image.bmWidth), ScalerHeigthPixel(btLoading_Image.bmHeight), &btLoading_Image);
		} else {
			Start_X = ( CSAPP_OSD_MAX_WIDTH - btLoading_Image.bmWidth ) / 2;
			MV_FillBoxWithBitmap (hdc, ScalerWidthPixel(Start_X), ScalerHeigthPixel(0), ScalerWidthPixel(btLoading_Image.bmWidth), ScalerHeigthPixel(btLoading_Image.bmHeight), &btLoading_Image);
		}
	} else {
		if ( btLoading_Image.bmHeight < CSAPP_OSD_MAX_HEIGHT )
		{
			Start_Y = ( CSAPP_OSD_MAX_HEIGHT - btLoading_Image.bmHeight ) / 2;
			MV_FillBoxWithBitmap (hdc, ScalerWidthPixel(0), ScalerHeigthPixel(Start_Y), ScalerWidthPixel(btLoading_Image.bmWidth), ScalerHeigthPixel(btLoading_Image.bmHeight), &btLoading_Image);
		} else {
			MV_FillBoxWithBitmap (hdc, ScalerWidthPixel(0), ScalerHeigthPixel(0), ScalerWidthPixel(btLoading_Image.bmWidth), ScalerHeigthPixel(btLoading_Image.bmHeight), &btLoading_Image);
		}
	}
	EndPaint(hwnd,hdc);
}

void Mv_Restore_Viewer(HWND hwnd)
{
	HDC			hdc;

	b8Image_Load_Flag = FALSE;
	hdc = BeginPaint(hwnd);
	MV_FillBoxWithBitmap (hdc, ScalerWidthPixel(0), ScalerHeigthPixel(0), ScalerWidthPixel(CSAPP_OSD_MAX_WIDTH), ScalerHeigthPixel(CSAPP_OSD_MAX_HEIGHT), &btBackup_Screen);
	EndPaint(hwnd,hdc);
}

void MV_Media_VolumeScroll(BOOL right)
{
	PVR_Volume.Volume=CS_DBU_GetVolume();

	if(CS_AV_Audio_GetMuteStatus() == TRUE)
		CS_AV_Audio_SetMuteStatus(FALSE);

	if(right == FALSE)
	{
		if(PVR_Volume.Volume<=0)
			PVR_Volume.Volume = 0;
		else
			PVR_Volume.Volume--;
	}
	else
	{
		PVR_Volume.Volume++;
		if(PVR_Volume.Volume > kCS_DBU_MAX_VOLUME)
			PVR_Volume.Volume = kCS_DBU_MAX_VOLUME;
	}

	if(PVR_Volume.Volume == 0)
	{
		CS_DBU_SetMuteStatus(eCS_DBU_ON);
		CS_AV_Audio_SetMuteStatus(TRUE);
		CS_DBU_SaveMuteStatus();
	}
	else
	{
		if(CS_AV_Audio_GetMuteStatus()==TRUE)
		{
			CS_DBU_SetMuteStatus(eCS_DBU_OFF);
			CS_AV_Audio_SetMuteStatus(FALSE);
			CS_DBU_SaveMuteStatus();
		}
	}

	PVR_Volume.Draw=TRUE;
	CS_DBU_SetVolume(PVR_Volume.Volume);

	CS_AV_AudioSetVolume(PVR_Volume.Volume);

	CS_DBU_SaveVolume();
	CS_DBU_SaveMuteStatus();

}

void MV_Media_PaintVolume(HDC hdc)
{
	int      	num;
	char		acText[5];

	if ( btCap_Volume.bmHeight == 0 )
	{
		MV_GetBitmapFromDC (hdc, ScalerWidthPixel(VOLUME_X - MV_BMP[MVBMP_VOLUME_ICON].bmWidth),ScalerHeigthPixel(VOLUME_Y),ScalerWidthPixel(VOLUME_FULL_DX),ScalerHeigthPixel(VOLUME_DY), &btCap_Volume);
	} else {
		FillBoxWithBitmap (hdc, ScalerWidthPixel(VOLUME_X - MV_BMP[MVBMP_VOLUME_ICON].bmWidth),ScalerHeigthPixel(VOLUME_Y),ScalerWidthPixel(VOLUME_FULL_DX),ScalerHeigthPixel(VOLUME_DY), &btCap_Volume);
	}

	if( PVR_Volume.Draw != TRUE && PVR_Volume.OnScreen == TRUE )
	{
		PVR_Volume.OnScreen=FALSE;
#if 0
		SetBrushColor(hdc, MVAPP_BACKBLUE_COLOR);
		FillBox(hdc,ScalerWidthPixel(VOLUME_X - MV_BMP[MVBMP_VOLUME_ICON].bmWidth),ScalerHeigthPixel(VOLUME_Y),ScalerWidthPixel(VOLUME_FULL_DX),ScalerHeigthPixel(VOLUME_DY));
#endif
		return;
	}

	PVR_Volume.OnScreen = TRUE;
	num = PVR_Volume.Volume;

	FillBoxWithBitmap (hdc, ScalerWidthPixel(VOLUME_X - MV_BMP[MVBMP_VOLUME_ICON].bmWidth), ScalerHeigthPixel(VOLUME_Y), ScalerWidthPixel(MV_BMP[MVBMP_VOLUME_ICON].bmWidth),ScalerHeigthPixel(MV_BMP[MVBMP_VOLUME_ICON].bmHeight), &MV_BMP[MVBMP_VOLUME_ICON]);

	SetTextColor(hdc,MV_BAR_UNFOCUS_CHAR_COLOR);
	SetBkMode(hdc,BM_TRANSPARENT);
	MV_SetBrushColor( hdc, MVAPP_BLACK_COLOR_ALPHA );
	FillBox(hdc,ScalerWidthPixel(VOLUME_X),ScalerHeigthPixel(VOLUME_Y),ScalerWidthPixel(VOLUME_DX + VOLUME_TEXT_DX),ScalerHeigthPixel(VOLUME_DY));
	MV_SetBrushColor(hdc,MVAPP_YELLOW_COLOR);
	FillBox(hdc,ScalerWidthPixel(VOLUME_X),ScalerHeigthPixel(VOLUME_Y+5),ScalerWidthPixel((VOLUME_DX * ((num*100)/kCS_DBU_MAX_VOLUME))/100),ScalerHeigthPixel(VOLUME_DY-10));

	sprintf(acText, "%d", num);
	MV_CS_MW_TextOut( hdc, ScalerWidthPixel(VOLUME_TEXT_X + 10),ScalerHeigthPixel(VOLUME_Y + 7), acText);
}

void MV_Media_PaintMute(HDC hdc)
{
	if(CS_AV_Audio_GetMuteStatus() == FALSE)
	{
		SetBrushColor(hdc, COLOR_transparent);
		FillBox(hdc,ScalerWidthPixel(MEDIA_MUTE_ICON_X), ScalerHeigthPixel(MEDIA_MUTE_ICON_Y), ScalerWidthPixel(MV_BMP[MVBMP_MUTE_ICON].bmWidth), ScalerHeigthPixel(MV_BMP[MVBMP_MUTE_ICON].bmHeight));
	}
	else
	{
		FillBoxWithBitmap (hdc, ScalerWidthPixel(MEDIA_MUTE_ICON_X), ScalerHeigthPixel(MEDIA_MUTE_ICON_Y),ScalerWidthPixel(MV_BMP[MVBMP_MUTE_ICON].bmWidth), ScalerHeigthPixel(MV_BMP[MVBMP_MUTE_ICON].bmHeight), &MV_BMP[MVBMP_MUTE_ICON]);
	}
}

CSAPP_Applet_t CSApp_MPlayer(void)
{
	int					BASE_X, BASE_Y, WIDTH, HEIGHT;
	MSG 				msg;
	HWND				hwndMain;
	MAINWINCREATE		CreateInfo;

	CSApp_Mplayer_Applets = CSApp_Applet_Error;

#ifdef  Screen_1080
	BASE_X = 0;
	BASE_Y = 0;
	WIDTH  = 1920;
	HEIGHT = 1080;
#else
	BASE_X = 0;
	BASE_Y = 0;
	WIDTH  = ScalerWidthPixel(CSAPP_OSD_MAX_WIDTH);
	HEIGHT = ScalerHeigthPixel(CSAPP_OSD_MAX_HEIGHT);
#endif

	CreateInfo.dwStyle		= WS_VISIBLE;
	CreateInfo.dwExStyle	= WS_EX_NONE;
	CreateInfo.spCaption	= "Media Player";
	CreateInfo.hMenu		= 0;
	CreateInfo.hCursor		= 0;
	CreateInfo.hIcon		= 0;
	CreateInfo.MainWindowProc = MPlayer_Msg_cb;
	CreateInfo.lx 			= BASE_X;
	CreateInfo.ty 			= BASE_Y;
	CreateInfo.rx 			= BASE_X+WIDTH;
	CreateInfo.by 			= BASE_Y+HEIGHT;
	CreateInfo.iBkColor 	= COLOR_transparent;
	CreateInfo.dwAddData 	= 0;
	CreateInfo.hHosting 	= HWND_DESKTOP;

	hwndMain = CreateMainWindow (&CreateInfo);

	if (hwndMain == HWND_INVALID)	return CSApp_Applet_Error;

	ShowWindow(hwndMain, SW_SHOWNORMAL);

	while (GetMessage(&msg, hwndMain))
	{
		TranslateMessage(&msg);
		DispatchMessage(&msg);
	}
	MainWindowThreadCleanup (hwndMain);

	return CSApp_Mplayer_Applets;
}

int MPlayer_Msg_cb(HWND hwnd,int message,WPARAM wparam,LPARAM lparam)
{
   	HDC 				hdc;

	switch(message)
	   	{
			case MSG_CREATE:
				memset(&btCap_Volume, 0x00, sizeof(BITMAP));
				u16Current_index = 0;
				u16Current_Page = 0;
				u16Current_Focus = 0;
				u8ThisHDD = 0;
				File_Delete_Flag = FALSE;
				File_Rename_Flag = FALSE;
				b8Image_Load_Flag = FALSE;
				memset(&stMP3_Tag, 0x00, sizeof(stMp3Tag));
				memset(acCurrent_Dir, 0x00, MAX_FILE_NAME_LENGTH);
				strncpy( acCurrent_Dir, stPartition_data[u8ThisHDD].acPartition_Dir, strlen(stPartition_data[u8ThisHDD].acPartition_Dir));
				MV_MPlayer_Load_fileData(acCurrent_Dir);
				break;

			case MSG_PAINT:
				CS_MW_SetSmallWindow(ScalerWidthPixel(MV_PIG_LEFT),ScalerHeigthPixel(MV_PIG_TOP),ScalerWidthPixel(MV_PIG_DX),ScalerHeigthPixel(MV_PIG_DY));
				MV_DRAWING_MENUBACK(hwnd, CSAPP_MAINMENU_MEDIA, EN_ITEM_FOCUS_MEDIA_PLAYER);

				hdc = BeginPaint(hwnd);
				MV_Draw_MPlayer_MenuTitle(hdc);
				MV_Draw_MPlayer_MenuBar(hdc);
				MV_Draw_MPlayer_FileInfo(hdc, u16Current_index);
				MV_Draw_MPlayer_StorageInfo(hdc);
				MV_Draw_MPlayer_Partition_Info(hdc);
				MV_Media_draw_help_banner(hdc, EN_ITEM_FOCUS_MEDIA_PLAYER);
				EndPaint(hwnd,hdc);

				return 0;

			case MSG_USB_MSG:
				if ( UsbCon_GetStatus() != USB_STATUS_MOUNTED )
				{
					CSApp_Mplayer_Applets=CSApp_Applet_MainMenu;
					SendMessage(hwnd,MSG_CLOSE,0,0);
				}
				break;

			case MSG_TIMER:
				if(wparam == DESKTOP_VOLUME_TIMER_ID)
				{
					PVR_Volume.Draw = FALSE;

					hdc=MV_BeginPaint(hwnd);
					MV_Media_PaintVolume(hdc);
					MV_EndPaint(hwnd,hdc);

					if(IsTimerInstalled(hwnd, DESKTOP_VOLUME_TIMER_ID))
						KillTimer(hwnd,DESKTOP_VOLUME_TIMER_ID);
				}
				else if(wparam == DESKTOP_BANNER_TIMER_ID)
				{
					if ( b8Mp3Play_Flag == TRUE )
					{
						hdc = BeginPaint(hwnd);
						MV_Mp3Progress(hdc);
						EndPaint(hwnd,hdc);
#if 0
						//printf("1=====\n");
						if(IsTimerInstalled(hwnd, DESKTOP_BANNER_TIMER_ID))
							KillTimer(hwnd, DESKTOP_BANNER_TIMER_ID);

						SetTimer(hwnd, DESKTOP_BANNER_TIMER_ID, Mp3_Progress_timer_Max);
						//printf("2=====\n");
#endif
					}
				}
				break;

			case MSG_KEYDOWN:
				if ( b8Image_Load_Flag == TRUE )
				{
					Mv_Restore_Viewer(hwnd);
					break;
				}

				if ( MV_Check_Confirm_Window() == TRUE )
				{
					struct stat		statbuffer;

					MV_Confirm_Proc(hwnd, wparam);

					if ( wparam == CSAPP_KEY_ESC || wparam == CSAPP_KEY_MENU || wparam == CSAPP_KEY_ENTER )
					{
						if ( wparam == CSAPP_KEY_ENTER )
						{
							if ( MV_Check_YesNo() == TRUE )
							{
								if ( File_Delete_Flag == TRUE )
								{
									char	Temp_Str[MAX_FILE_NAME_LENGTH];

									memset(Temp_Str, 0x00, MAX_FILE_NAME_LENGTH);
									sprintf(Temp_Str, "%s/%s", acCurrent_Dir, stFileDB.acFileName[u16Current_index]);
									if( stat(Temp_Str, &statbuffer ) != 0 )
									{
										printf("MAIN STAT ERROR========================\n");
										break;
									}

									if( S_ISDIR(statbuffer.st_mode) == 0 )
									{
										memset(Temp_Str, 0x00, MAX_FILE_NAME_LENGTH);
										sprintf(Temp_Str, "rm %s/%s", acCurrent_Dir, stFileDB.acFileName[u16Current_index]);
										system(Temp_Str);

										if ( u16Current_index == stFileDB.u16file_Count )
											u16Current_index--;

										if ( strcmp ( stFileDB.acFileName[u16Current_index], "..") == 0 )
											break;

										MV_MPlayer_Load_fileData(acCurrent_Dir);

										hdc = BeginPaint(hwnd);
										Restore_Confirm_Window(hdc);
										MV_Draw_MPlayer_MenuTitle(hdc);
										MV_Draw_MPlayer_MenuBar(hdc);
										MV_Draw_MPlayer_FileInfo(hdc, u16Current_index);
										MV_Draw_MPlayer_StorageInfo(hdc);
										EndPaint(hwnd,hdc);
									}
									File_Delete_Flag = FALSE;
								}
							}else {
								hdc = BeginPaint(hwnd);
								Restore_Confirm_Window(hdc);
								EndPaint(hwnd,hdc);
								File_Delete_Flag = FALSE;
							}
						} else {
							hdc = BeginPaint(hwnd);
							Restore_Confirm_Window(hdc);
							EndPaint(hwnd,hdc);
							File_Delete_Flag = FALSE;
						}
					}

					if (wparam != CSAPP_KEY_IDLE)
					{
						break;
					}
				}

				if ( Get_Keypad_Status() == TRUE )
				{
					char			sReturn_str[MAX_FILE_NAME_LENGTH];
					char			Temp_Str[MAX_FILE_NAME_LENGTH];
					struct stat		statbuffer;

					memset( sReturn_str, 0x00, MAX_FILE_NAME_LENGTH );

					UI_Keypad_Proc(hwnd, wparam);

					if ( wparam == CSAPP_KEY_ESC || wparam == CSAPP_KEY_MENU || wparam == CSAPP_KEY_ENTER || wparam == CSAPP_KEY_YELLOW )
					{
						if ( wparam == CSAPP_KEY_ENTER || wparam == CSAPP_KEY_YELLOW )
						{
							if ( Get_Keypad_is_Save() == TRUE )
							{
								Get_Save_Str(sReturn_str);

								sprintf(Temp_Str, "%s/%s", acCurrent_Dir, stFileDB.acFileName[u16Current_index]);

								if( stat(Temp_Str, &statbuffer ) != 0 )
								{
									printf("MAIN STAT ERROR========================\n");
									break;
								}

								if( S_ISDIR(statbuffer.st_mode) )
									sprintf(Temp_Str, "mv %s/%s/ %s/%s/", acCurrent_Dir, stFileDB.acFileName[u16Current_index], acCurrent_Dir, sReturn_str);
								else
									sprintf(Temp_Str, "mv %s/%s %s/%s.%s", acCurrent_Dir, stFileDB.acFileName[u16Current_index], acCurrent_Dir, sReturn_str, stFileDB.acFileExt[u16Current_index]);

								printf("=== File Rename : %s\n", Temp_Str);
								system(Temp_Str);

								hdc = BeginPaint(hwnd);
								MV_MPlayer_Load_fileData(acCurrent_Dir);
								MV_Draw_MPlayer_MenuBar(hdc);
								EndPaint(hwnd,hdc);
							}
						}
						File_Rename_Flag = FALSE;
					}
					break;
				}

				switch(wparam)
				{
					case CSAPP_KEY_RED:
						if ( acUsbDevCount > 0 )
						{
							if ( u8ThisHDD == 0 )
								u8ThisHDD = acUsbDevCount - 1;
							else
								u8ThisHDD--;

							u16Current_index = 0;
							u16Current_Page = 0;
							u16Current_Focus = 0;
							File_Delete_Flag = FALSE;
							File_Rename_Flag = FALSE;
							b8Image_Load_Flag = FALSE;
							memset(&stMP3_Tag, 0x00, sizeof(stMp3Tag));
							memset(acCurrent_Dir, 0x00, MAX_FILE_NAME_LENGTH);
							strncpy( acCurrent_Dir, stPartition_data[u8ThisHDD].acPartition_Dir, strlen(stPartition_data[u8ThisHDD].acPartition_Dir));
							MV_MPlayer_Load_fileData(acCurrent_Dir);

							hdc = BeginPaint(hwnd);
							MV_Draw_MPlayer_MenuTitle(hdc);
							MV_Draw_MPlayer_MenuBar(hdc);
							MV_Draw_MPlayer_FileInfo(hdc, u16Current_index);
							MV_Draw_MPlayer_StorageInfo(hdc);
							MV_Draw_MPlayer_Partition_Info(hdc);
							EndPaint(hwnd,hdc);
						}
						break;

					case CSAPP_KEY_BLUE:
						if ( acUsbDevCount > 0 )
						{
							u8ThisHDD++;
							if ( u8ThisHDD >= acUsbDevCount )
								u8ThisHDD = 0;

							u16Current_index = 0;
							u16Current_Page = 0;
							u16Current_Focus = 0;
							File_Delete_Flag = FALSE;
							File_Rename_Flag = FALSE;
							b8Image_Load_Flag = FALSE;
							memset(&stMP3_Tag, 0x00, sizeof(stMp3Tag));
							memset(acCurrent_Dir, 0x00, MAX_FILE_NAME_LENGTH);
							strncpy( acCurrent_Dir, stPartition_data[u8ThisHDD].acPartition_Dir, strlen(stPartition_data[u8ThisHDD].acPartition_Dir));
							MV_MPlayer_Load_fileData(acCurrent_Dir);

							hdc = BeginPaint(hwnd);
							MV_Draw_MPlayer_MenuTitle(hdc);
							MV_Draw_MPlayer_MenuBar(hdc);
							MV_Draw_MPlayer_FileInfo(hdc, u16Current_index);
							MV_Draw_MPlayer_StorageInfo(hdc);
							MV_Draw_MPlayer_Partition_Info(hdc);
							EndPaint(hwnd,hdc);
						}
						break;

					case CSAPP_KEY_MUTE:
						if(CS_AV_Audio_GetMuteStatus() == FALSE)
							CS_AV_Audio_SetMuteStatus(TRUE);
						else
							CS_AV_Audio_SetMuteStatus(FALSE);

						CS_DBU_SaveMuteStatus();

						hdc=MV_BeginPaint(hwnd);
						MV_Media_PaintMute(hdc);
						MV_EndPaint(hwnd,hdc);
						break;

					case CSAPP_KEY_STOP:
						if ( b8Mp3Play_Flag == TRUE )
						{
							printf("\n==== PRESS STOP KEY =====\n");
							if(IsTimerInstalled(hwnd, DESKTOP_BANNER_TIMER_ID))
								KillTimer(hwnd, DESKTOP_BANNER_TIMER_ID);
							MV_Mp3Stop();
							hdc=MV_BeginPaint(hwnd);
							MV_Draw_MPlayer_StorageInfo(hdc);
							MV_EndPaint(hwnd,hdc);
						}
						break;

					case CSAPP_KEY_UP:
						hdc = BeginPaint(hwnd);
						u16Prev_Page = u16Current_Page;
						MV_Draw_MPlayerMenuBar(hdc, u16Current_Focus, UNFOCUS);

						if ( u16Current_index <= 0 )
							u16Current_index = stFileDB.u16file_Count - 1;
						else
							u16Current_index--;

						u16Current_Focus = get_focus_line(&u16Current_Page, u16Current_index, LIST_MAX_ITEM);

						if ( u16Prev_Page != u16Current_Page )
							MV_Draw_MPlayer_MenuBar(hdc);
						else
							MV_Draw_MPlayerMenuBar(hdc, u16Current_Focus, FOCUS);
						//EndPaint(hwnd,hdc);

						//hdc = BeginPaint(hwnd);
						MV_Draw_MPlayer_MenuTitle(hdc);
						MV_Draw_MPlayer_FileInfo(hdc, u16Current_index);
						//MV_Draw_MPlayer_StorageInfo(hdc);
						EndPaint(hwnd,hdc);
						break;

					case CSAPP_KEY_DOWN:
						hdc = BeginPaint(hwnd);
						u16Prev_Page = u16Current_Page;
						MV_Draw_MPlayerMenuBar(hdc, u16Current_Focus, UNFOCUS);

						if ( u16Current_index >= stFileDB.u16file_Count - 1 )
							u16Current_index = 0;
						else
							u16Current_index++;

						u16Current_Focus = get_focus_line(&u16Current_Page, u16Current_index, LIST_MAX_ITEM);

						if ( u16Prev_Page != u16Current_Page )
							MV_Draw_MPlayer_MenuBar(hdc);
						else
							MV_Draw_MPlayerMenuBar(hdc, u16Current_Focus, FOCUS);

						//EndPaint(hwnd,hdc);

						//hdc = BeginPaint(hwnd);
						MV_Draw_MPlayer_MenuTitle(hdc);
						MV_Draw_MPlayer_FileInfo(hdc, u16Current_index);
						//MV_Draw_MPlayer_StorageInfo(hdc);
						EndPaint(hwnd,hdc);
						break;

					case CSAPP_KEY_VOL_DOWN:
					case CSAPP_KEY_LEFT:
						MV_Media_VolumeScroll(FALSE);

						hdc=MV_BeginPaint(hwnd);
						MV_Media_PaintVolume(hdc);
						MV_Media_PaintMute(hdc);
						MV_EndPaint(hwnd,hdc);

						if(IsTimerInstalled(hwnd, DESKTOP_VOLUME_TIMER_ID))
							KillTimer(hwnd, DESKTOP_VOLUME_TIMER_ID);

						SetTimer(hwnd, DESKTOP_VOLUME_TIMER_ID, Volume_timer_Max);
						break;

					case CSAPP_KEY_VOL_UP:
					case CSAPP_KEY_RIGHT:
						MV_Media_VolumeScroll(TRUE);

						hdc=MV_BeginPaint(hwnd);
						MV_Media_PaintVolume(hdc);
						MV_Media_PaintMute(hdc);
						MV_EndPaint(hwnd,hdc);

						if(IsTimerInstalled(hwnd, DESKTOP_VOLUME_TIMER_ID))
							KillTimer(hwnd, DESKTOP_VOLUME_TIMER_ID);

						SetTimer(hwnd, DESKTOP_VOLUME_TIMER_ID, Volume_timer_Max);
						break;

					case CSAPP_KEY_ENTER:
						{
							struct stat		statbuffer;
							char			Temp_str[MAX_FILE_NAME_LENGTH];

							memset(Temp_str, 0x00, MAX_FILE_NAME_LENGTH );

							sprintf(Temp_str, "%s/%s", acCurrent_Dir, stFileDB.acFileName[u16Current_index]);

							//printf("\n\n===== Dir : %s\n\n", Temp_str);

							if( stat(Temp_str, &statbuffer ) != 0 )
							{
								printf("MAIN STAT ERROR ========================\n");
								break;
							} else {
								//printf("FILE STAT OK ========================\n");
								if( MV_Check_Filetype(stFileDB.acFileExt[u16Current_index]) == MVAPP_FILE_PIC )
								{
									//printf("FILE is IMAGE File ========================\n");
									switch( Check_Picture_Kind(stFileDB.acFileExt[u16Current_index]) )
									{
										case 0:
											Mv_Image_Viewer(hwnd, &stFileDB);
											printf("====== This is JPG File =======\n");
											break;
										case 1:
											Mv_Image_Viewer(hwnd, &stFileDB);
											printf("====== This is PNG File =======\n");
											break;
										case 2:
											Mv_Image_Viewer(hwnd, &stFileDB);
											printf("====== This is GIF File =======\n");
											break;
										case 3:
											Mv_Image_Viewer(hwnd, &stFileDB);
											printf("====== This is BMP File =======\n");
											break;
										default:
											break;
									}
								}else if ( MV_Check_Filetype(stFileDB.acFileExt[u16Current_index]) == MVAPP_FILE_MUSIC )
								{
									if(IsTimerInstalled(hwnd, DESKTOP_BANNER_TIMER_ID))
										KillTimer(hwnd, DESKTOP_BANNER_TIMER_ID);

									SetTimer(hwnd, DESKTOP_BANNER_TIMER_ID, Mp3_Progress_timer_Max);

									MV_Mp3Play(Temp_str);

									hdc=MV_BeginPaint(hwnd);
									MV_Draw_MPlayer_StorageInfo(hdc);
									EndPaint(hwnd,hdc);
								}
								//printf("FILE STAT OK========================\n");
							}

							if( S_ISDIR(statbuffer.st_mode) )
							{
								if ( strcmp( stFileDB.acFileName[u16Current_index] , "..") == 0 )
								{
									MV_MPlayer_Under_Dir();
									MV_MPlayer_Load_fileData(acCurrent_Dir);
								} else {
									strncpy(acCurrent_Dir, Temp_str, strlen(Temp_str));
									MV_MPlayer_Load_fileData(acCurrent_Dir);
								}

								u16Current_index = 0;
								u16Current_Page = 0;
								u16Current_Focus = 0;

								hdc = BeginPaint(hwnd);
								MV_Draw_MPlayer_MenuTitle(hdc);
								MV_Draw_MPlayer_MenuBar(hdc);
								MV_Draw_MPlayer_FileInfo(hdc, u16Current_index);
								MV_Draw_MPlayer_StorageInfo(hdc);
								EndPaint(hwnd,hdc);
							}
						}
						break;

					case CSAPP_KEY_ESC:
						if ( PVR_Volume.OnScreen == TRUE )
						{
							PVR_Volume.Draw = FALSE;

							hdc=MV_BeginPaint(hwnd);
							MV_Media_PaintVolume(hdc);
							MV_EndPaint(hwnd,hdc);

							if(IsTimerInstalled(hwnd, DESKTOP_VOLUME_TIMER_ID))
								KillTimer(hwnd, DESKTOP_VOLUME_TIMER_ID);

							break;
						}
						else if ( b8Mp3Play_Flag == TRUE )
						{
							printf("\n==== PRESS EXIT KEY =====\n");
							if(IsTimerInstalled(hwnd, DESKTOP_BANNER_TIMER_ID))
								KillTimer(hwnd, DESKTOP_BANNER_TIMER_ID);
							MV_Mp3Stop();
							hdc=MV_BeginPaint(hwnd);
							MV_Draw_MPlayer_StorageInfo(hdc);
							MV_EndPaint(hwnd,hdc);
							break;
						}
						else if ( strcmp(acCurrent_Dir, stPartition_data[u8ThisHDD].acPartition_Dir) != 0 )
						{
							MV_MPlayer_Under_Dir();
							MV_MPlayer_Load_fileData(acCurrent_Dir);

							u16Current_index = 0;
							u16Current_Page = 0;
							u16Current_Focus = 0;

							hdc = BeginPaint(hwnd);
							MV_Draw_MPlayer_MenuTitle(hdc);
							MV_Draw_MPlayer_MenuBar(hdc);
							MV_Draw_MPlayer_FileInfo(hdc, u16Current_index);
							MV_Draw_MPlayer_StorageInfo(hdc);
							EndPaint(hwnd,hdc);
							break;
						}

						if(IsTimerInstalled(hwnd, DESKTOP_VOLUME_TIMER_ID))
							KillTimer(hwnd,DESKTOP_VOLUME_TIMER_ID);

						CSApp_Mplayer_Applets=CSApp_Applet_Desktop;
						SendMessage(hwnd,MSG_CLOSE,0,0);
						break;

					case CSAPP_KEY_MENU:
						if ( PVR_Volume.OnScreen == TRUE )
						{
							PVR_Volume.Draw = FALSE;

							hdc=MV_BeginPaint(hwnd);
							MV_Media_PaintVolume(hdc);
							MV_EndPaint(hwnd,hdc);

							if(IsTimerInstalled(hwnd, DESKTOP_VOLUME_TIMER_ID))
								KillTimer(hwnd, DESKTOP_VOLUME_TIMER_ID);

							break;
						}
						else if ( b8Mp3Play_Flag == TRUE )
						{
							printf("\n==== PRESS EXIT KEY =====\n");
							if(IsTimerInstalled(hwnd, DESKTOP_BANNER_TIMER_ID))
								KillTimer(hwnd, DESKTOP_BANNER_TIMER_ID);
							MV_Mp3Stop();
							hdc=MV_BeginPaint(hwnd);
							MV_Draw_MPlayer_StorageInfo(hdc);
							MV_EndPaint(hwnd,hdc);
							break;
						}
						else if ( strcmp(acCurrent_Dir, stPartition_data[u8ThisHDD].acPartition_Dir) != 0 )
						{
							MV_MPlayer_Under_Dir();
							MV_MPlayer_Load_fileData(acCurrent_Dir);

							u16Current_index = 0;
							u16Current_Page = 0;
							u16Current_Focus = 0;

							hdc = BeginPaint(hwnd);
							MV_Draw_MPlayer_MenuTitle(hdc);
							MV_Draw_MPlayer_MenuBar(hdc);
							MV_Draw_MPlayer_FileInfo(hdc, u16Current_index);
							MV_Draw_MPlayer_StorageInfo(hdc);
							EndPaint(hwnd,hdc);
							break;
						}

						if(IsTimerInstalled(hwnd, DESKTOP_VOLUME_TIMER_ID))
							KillTimer(hwnd,DESKTOP_VOLUME_TIMER_ID);

						CSApp_Mplayer_Applets=b8Last_App_Status;
						SendMessage(hwnd,MSG_CLOSE,0,0);
						break;

					case CSAPP_KEY_IDLE:
						CSApp_Mplayer_Applets = CSApp_Applet_Sleep;
						SendMessage(hwnd,MSG_CLOSE,0,0);
						break;

					case CSAPP_KEY_TV_AV:
						ScartSbOnOff(); /* By KB Kim : 2010_08_31 for Scart Control */
						break;

					default:
						break;
				}
				break;

		   	case MSG_CLOSE:
				UnloadBitmap(&btCap_Volume);
				CS_MW_SetNormalWindow();
				DestroyMainWindow(hwnd);
				PostQuitMessage(hwnd);
				break;

		   	default:
				break;
	   	}

   return DefaultMainWinProc(hwnd,message,wparam,lparam);
}



