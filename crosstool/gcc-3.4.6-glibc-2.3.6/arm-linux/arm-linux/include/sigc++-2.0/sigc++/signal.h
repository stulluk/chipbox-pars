...
struct signal_emit0
{
   ...
   */
  static result_type emit(const iterator_type& first, const iterator_type& last)
    {
      self_type self ;
...
struct signal_emit0<T_return, nil>
{
   ...
   */
  static result_type emit(iterator_type first, iterator_type last)
    {
      ...
      T_return r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_);
      for (++first; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_);
        }
      return r_;
    }
};
      ...
      for (; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_);
        }
    }
};
...
struct signal_emit1
{
   ...
   * @return The accumulated return values of the slot invocations as processed by the accumulator.
   */
  static result_type emit(const iterator_type& first, const iterator_type& last, typename type_trait<T_arg1>::take _A_a1)
    {
...
struct signal_emit1<T_return, T_arg1, nil>
{
   ...
   * @return The return value of the last slot invoked.
   */
  static result_type emit(iterator_type first, iterator_type last, typename type_trait<T_arg1>::take _A_a1)
    {
      for (; first != last; ++first) if (!first->empty() && !first->blocked()) break;
      if (first == last) return T_return(); // avoid compiler warning about r_ being possibly uninitialized (T_return r_(); doesn't work)

      T_return r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1);
      for (++first; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1);
        }
      return r_;
    }
};
      ...
      for (; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1);
        }
    }
};
...
struct signal_emit2
{
   ...
   * @return The accumulated return values of the slot invocations as processed by the accumulator.
   */
  static result_type emit(const iterator_type& first, const iterator_type& last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2)
    {
...
struct signal_emit2<T_return, T_arg1,T_arg2, nil>
{
   ...
   * @return The return value of the last slot invoked.
   */
  static result_type emit(iterator_type first, iterator_type last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2)
    {
      for (; first != last; ++first) if (!first->empty() && !first->blocked()) break;
      if (first == last) return T_return(); // avoid compiler warning about r_ being possibly uninitialized (T_return r_(); doesn't work)

      T_return r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2);
      for (++first; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2);
        }
      return r_;
    }
};
      ...
      for (; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2);
        }
    }
};
...
struct signal_emit3
{
   ...
   * @return The accumulated return values of the slot invocations as processed by the accumulator.
   */
  static result_type emit(const iterator_type& first, const iterator_type& last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3)
    {
...
struct signal_emit3<T_return, T_arg1,T_arg2,T_arg3, nil>
{
   ...
   * @return The return value of the last slot invoked.
   */
  static result_type emit(iterator_type first, iterator_type last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3)
    {
      for (; first != last; ++first) if (!first->empty() && !first->blocked()) break;
      if (first == last) return T_return(); // avoid compiler warning about r_ being possibly uninitialized (T_return r_(); doesn't work)

      T_return r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3);
      for (++first; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3);
        }
      return r_;
    }
};
      ...
      for (; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3);
        }
    }
};
...
struct signal_emit4
{
   ...
   * @return The accumulated return values of the slot invocations as processed by the accumulator.
   */
  static result_type emit(const iterator_type& first, const iterator_type& last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4)
    {
...
struct signal_emit4<T_return, T_arg1,T_arg2,T_arg3,T_arg4, nil>
{
   ...
   * @return The return value of the last slot invoked.
   */
  static result_type emit(iterator_type first, iterator_type last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4)
    {
      for (; first != last; ++first) if (!first->empty() && !first->blocked()) break;
      if (first == last) return T_return(); // avoid compiler warning about r_ being possibly uninitialized (T_return r_(); doesn't work)

      T_return r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4);
      for (++first; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4);
        }
      return r_;
    }
};
      ...
      for (; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4);
        }
    }
};
...
struct signal_emit5
{
   ...
   * @return The accumulated return values of the slot invocations as processed by the accumulator.
   */
  static result_type emit(const iterator_type& first, const iterator_type& last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4,typename type_trait<T_arg5>::take _A_a5)
    {
...
struct signal_emit5<T_return, T_arg1,T_arg2,T_arg3,T_arg4,T_arg5, nil>
{
   ...
   * @return The return value of the last slot invoked.
   */
  static result_type emit(iterator_type first, iterator_type last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4,typename type_trait<T_arg5>::take _A_a5)
    {
      for (; first != last; ++first) if (!first->empty() && !first->blocked()) break;
      if (first == last) return T_return(); // avoid compiler warning about r_ being possibly uninitialized (T_return r_(); doesn't work)

      T_return r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4,_A_a5);
      for (++first; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4,_A_a5);
        }
      return r_;
    }
};
      ...
      for (; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4,_A_a5);
        }
    }
};
...
struct signal_emit6
{
   ...
   * @return The accumulated return values of the slot invocations as processed by the accumulator.
   */
  static result_type emit(const iterator_type& first, const iterator_type& last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4,typename type_trait<T_arg5>::take _A_a5,typename type_trait<T_arg6>::take _A_a6)
    {
...
struct signal_emit6<T_return, T_arg1,T_arg2,T_arg3,T_arg4,T_arg5,T_arg6, nil>
{
   ...
   * @return The return value of the last slot invoked.
   */
  static result_type emit(iterator_type first, iterator_type last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4,typename type_trait<T_arg5>::take _A_a5,typename type_trait<T_arg6>::take _A_a6)
    {
      for (; first != last; ++first) if (!first->empty() && !first->blocked()) break;
      if (first == last) return T_return(); // avoid compiler warning about r_ being possibly uninitialized (T_return r_(); doesn't work)

      T_return r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4,_A_a5,_A_a6);
      for (++first; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4,_A_a5,_A_a6);
        }
      return r_;
    }
};
      ...
      for (; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4,_A_a5,_A_a6);
        }
    }
};
...
struct signal_emit7<T_return, T_arg1,T_arg2,T_arg3,T_arg4,T_arg5,T_arg6,T_arg7, nil>
{
   ...
   * @return The return value of the last slot invoked.
   */
  static result_type emit(iterator_type first, iterator_type last, typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4,typename type_trait<T_arg5>::take _A_a5,typename type_trait<T_arg6>::take _A_a6,typename type_trait<T_arg7>::take _A_a7)
    {
      for (; first != last; ++first) if (!first->empty() && !first->blocked()) break;
      if (first == last) return T_return(); // avoid compiler warning about r_ being possibly uninitialized (T_return r_(); doesn't work)

      T_return r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4,_A_a5,_A_a6,_A_a7);
      for (++first; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          r_ = (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4,_A_a5,_A_a6,_A_a7);
        }
      return r_;
    }
};
      ...
      for (; first != last; ++first)
        {
          if (first->empty() || first->blocked())
            continue;
          (reinterpret_cast<call_type>(first->rep_->call_))(first->rep_, _A_a1,_A_a2,_A_a3,_A_a4,_A_a5,_A_a6,_A_a7);
        }
    }
};
...
public:
  ...
  result_type emit() const
    {
      ...
      return emitter_type::emit(impl_->slots_.begin(), impl_->slots_.end());
    }
  ...
  const slot_list slots() const
    { return slot_list(const_cast<signal0*>(this)->impl()); }
...
public:
  ...
  result_type emit(typename type_trait<T_arg1>::take _A_a1) const
    {
      if (empty())
        return result_type();
      internal::signal_exec exec(impl_);
      return emitter_type::emit(impl_->slots_.begin(), impl_->slots_.end(), _A_a1);
    }
   ...
   */
  const slot_list slots() const
    { return slot_list(const_cast<signal1*>(this)->impl()); }
...
public:
  ...
  result_type emit(typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2) const
    {
      if (empty())
        return result_type();
      internal::signal_exec exec(impl_);
      return emitter_type::emit(impl_->slots_.begin(), impl_->slots_.end(), _A_a1,_A_a2);
    }
   ...
   */
  const slot_list slots() const
    { return slot_list(const_cast<signal2*>(this)->impl()); }
...
public:
  ...
  result_type emit(typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3) const
    {
      if (empty())
        return result_type();
      internal::signal_exec exec(impl_);
      return emitter_type::emit(impl_->slots_.begin(), impl_->slots_.end(), _A_a1,_A_a2,_A_a3);
    }
   ...
   */
  const slot_list slots() const
    { return slot_list(const_cast<signal3*>(this)->impl()); }
...
public:
  ...
  result_type emit(typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4) const
    {
      if (empty())
        return result_type();
      internal::signal_exec exec(impl_);
      return emitter_type::emit(impl_->slots_.begin(), impl_->slots_.end(), _A_a1,_A_a2,_A_a3,_A_a4);
    }
   ...
   */
  const slot_list slots() const
    { return slot_list(const_cast<signal4*>(this)->impl()); }
...
public:
  ...
  result_type emit(typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4,typename type_trait<T_arg5>::take _A_a5) const
    {
      if (empty())
        return result_type();
      internal::signal_exec exec(impl_);
      return emitter_type::emit(impl_->slots_.begin(), impl_->slots_.end(), _A_a1,_A_a2,_A_a3,_A_a4,_A_a5);
    }
   ...
   */
  const slot_list slots() const
    { return slot_list(const_cast<signal5*>(this)->impl()); }
...
public:
  ...
  result_type emit(typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4,typename type_trait<T_arg5>::take _A_a5,typename type_trait<T_arg6>::take _A_a6) const
    {
      if (empty())
        return result_type();
      internal::signal_exec exec(impl_);
      return emitter_type::emit(impl_->slots_.begin(), impl_->slots_.end(), _A_a1,_A_a2,_A_a3,_A_a4,_A_a5,_A_a6);
    }
   ...
   */
  const slot_list slots() const
    { return slot_list(const_cast<signal6*>(this)->impl()); }
...
public:
  ...
  result_type emit(typename type_trait<T_arg1>::take _A_a1,typename type_trait<T_arg2>::take _A_a2,typename type_trait<T_arg3>::take _A_a3,typename type_trait<T_arg4>::take _A_a4,typename type_trait<T_arg5>::take _A_a5,typename type_trait<T_arg6>::take _A_a6,typename type_trait<T_arg7>::take _A_a7) const
    {
      if (empty())
        return result_type();
      internal::signal_exec exec(impl_);
      return emitter_type::emit(impl_->slots_.begin(), impl_->slots_.end(), _A_a1,_A_a2,_A_a3,_A_a4,_A_a5,_A_a6,_A_a7);
    }
   ...
   */
  const slot_list slots() const
    { return slot_list(const_cast<signal7*>(this)->impl()); }
